...
export const REQUISITOS =
    [
        ...
        new Item(Keys.REQUISITOS_QUALITY,
            [
        ...
        new Item(Keys.REQUISITOS_REQUIREMENTS_CHANGE,
            [
        ...
        new Item(Keys.REQUISITOS_PRAGMATIC_ENGINEER,
            [
                '- Un ingeniero pragmatico encuentra la mejor solución posible dentro de las limitaciones del mundo real, no del mundo ideal ',
                '',
                '- Desarrollar aplicaciones no solo implica escribir codigo, sino que un sinfin de variables, compensaciones, requisitos, ',
                ' - limitaciones, que se deben tener en cuenta, además de los recursos disponibles, el esfuerzo humano y la cultura organizacional ',
                '',
                '- Esto es especialmente importante en los tiempos actuales, donde la <strong>IA</strong> ayuda a encontrar soluciones de maneras más eficientes, ',
                ' - pero el toque de la experiencia humana siempre sera necesario, esta nos ayuda a evaluar y escoger la mejor alternativa ',
                '',
                '- Existen muchas soluciones diferentes para cada problema, que tecnicamente pueden ser correctas, pero en la practica suelen ',
                ' - tener problemas y/o limitaciones, los sistemas perfectos no existen y las soluciones no siempre deben ser idealizadas ',
                '',
                '<strong>Principios:</strong>',
                '<strong>Build the right thing, then build the thing right</strong>',
                '- De nada sirve construir una solución tecnicamente robusta si no presenta alguna utilidad de verdad, en especial si no resuelve nada ',
                ' - una correcta priorización es esencial para saber en que centrarse y en que no, maximizando el valor generado ',
                '',
                '<strong>Priorizar la simpleza sobre la complejidad</strong>',
                '- Todo codigo que exista, debe ser mantenido, cuanto más complejo sea, más dificil sera lograrlo ',
                '- Un codigo con demasiada abstracción, sobreingenieria o dificil de mantener, suele ser tan contraproducente como un codigo con malas practicas ',
                '',
                '<strong>Compensaciones y ganancias</strong>',
                '- En ingenieria suelen haber pocas decisiones que se puedan calificar como correctas/incorrectas, cada decisión suele implicar un equilibrio ',
                '- Soluciones que funcionan en un caso especifico, no necesariamente pueden servir en otros contextos, cada contexto es unico ',
                '',
                '<strong>Diseñar para el cambio, sin caer en la optimización prematura</strong>',
                '- <em>La optimización prematura es la raíz de todos los males</em>, <strong>Donald Knuth</strong> Optimizar demasiado pronto, en lugar de cuando sea necesario ',
                ' - consume recursos de manera anticipada y ataca un problema que no existe, es ideal siempre tener en cuenta principios como <strong>KISS</strong> y <strong>YAGNI</strong>',
                '',
                '<strong>Utilizar tecnologias estables, sobre tecnologias emergentes</strong>',
                '- Utilizar tecnologia estable y <em>"aburrida"</em> no es malo, como tampoco lo es buscar <em>innovación</em> de manera constante ',
                '- Se trata de un equilibrio donde se pueda escoger en base a las necesidades y que requiere el negocio ',
                '',
                '<strong>Sin miedo a las preguntas</strong>',
                '- Hacer las preguntas correctas antes de empezar a construir cualquier cosa, puede evitar errores y dolores de cabeza ',
                '- Requisitos críticos o limitaciones importantes pueden descubrirse a partir de preguntas simples y oportunas ',
                '',
                '<strong>Fail fast & lean startup</strong>',
                '- <em>La experiencia es simplemente el nombre que damos a nuestros errores</em>, <strong>Oscar Wilde</strong> fallar no esta mal ',
                ' - siempre y cuando quede algun aprendizaje, y nos evite volver a cometer los mismos errores ',
                '- Fracasar rápido es una estrategia que se centra en probar y refinar rápidamente las ideas para obtener información de los errores ',
                ' - en lugar de invertir tiempo excesivo en perfeccionar algo antes de validar su viabilidad, tanto a nivel técnico como de negocio ',
            ])
    ];
...
export const VERSIONAMIENTO =
    [
        ...
        new Item(Keys.VERSIONAMIENTO_MANAGEMENT_DEPENDENCIES,
            [
                '- Una dependencia es cualquier componente externo que un proyecto requiere para su correcto funcionamiento ',
                '- Los gestores de dependencias son ampliamente usados ya que evitan dolores de cabeza al manejar codigo de terceros o de externos ',
                '- Son herramientas, plugins, librerias, frameworks, necesarios para desarrollar aplicaciones de gran envergadura ',
                '',
                '<strong>Tipos de dependencias:</strong>',
                '<strong>- Dependencias directas:</strong> Estrictamente necesarias para el proyecto como una biblioteca o un framework ',
                '<strong>- Dependencias transitivas:</strong> Requeridas indirectamente, a menudo debido a que una dependencia requiere de otra dependencia ',
                '<strong>- Dependencias de desarrollo:</strong> Solo son utilizadas durante el desarrollo, no se necesitan en ambientes posteriores como producción ',
                '<strong>- Dependencias de tiempo de Ejecución:</strong> Son necesarias para que la aplicación pueda ejecutarse correctamente, como drivers de una BD o APIs ',
                '',
                '<strong>¿ Porque es importante ?</strong>',
                '- Seguridad, el codigo externo puede introducir vulnerabilidades, especialmente si las dependencias no se actualizan ',
                '- Estabilidad, al mantener solo versiones compatibles entre si, evitando cambios disruptivos ',
                '- Costos de mantenimiento reducido al hacer actualizaciones periodicas, al gestionar actualizaciones y conflictos de manera proactiva ',
            ]),
        ...
        new Item(Keys.VERSIONAMIENTO_DOCUMENTATION,
            [
                ...
                '- Esta debe formar parte de las estrategias de CI/CD que se tengan del proyecto, ya que tambien puede ser versionada sin problemas ',
        ...
        new Item(Keys.VERSIONAMIENTO_EVOLUTION_SCHEMA_AND_SCHEMA_REGISTRY,
            [
                '- El flujo de datos en las aplicaciones modernas es vital para todo el sistema y cualquier interrupción puede ser critica para el negocio ',
                '- La gestión de los cambios en tiempo real y sin interrupciones es un reto posible mediante diversas herramientas y estrategias ',
                '- En un sistema en tiempo real los esquemas no solo son detalles tecnicos, son contratos que definen propiedades y estructuran comportamientos ',
                '- La evolución es natural y sucede por multiples razones, principalmente negocio y necesidades tecnicas ',
                '',
                '- Lo que puede parecer un cambio trivial, un cambio de tipo, agregar/borrar/renombrar un atributo puede generar efectos inesperados y destructivos ',
                '- Evolucionar un esquema es preparar a un sistema ante los cambios de manera que la estructura de los datos sea flexible en el tiempo ',
                '- En un sistema que trabaje por lotes, es posible parar y ajustar los cambios, las pausas no afectan tanto la calidad de los datos ',
                '- Los sistemas en tiempo real son <em>fragiles</em> debido a que los datos antiguos simplemente no se pueden "reintentar", una vez fluyen los datos erroneos ',
                ' - se propagan instantaneamente y la prevención no siempre contiene los problemas ',
                '',
                '<strong>Buenas practicas',
                '&bull; Para garantizar la interoperabilidad de los sistemas es fundamental que tipo de compatibilidad se desea (Forward, Backward, Full)',
                '&bull; Utilice el versionamiento semantico de forma que facilite su seguimiento y la naturaleza de los cambios sin ahondar demasiado ',
                '&bull; Los valores predeterminados son determinantes para evitar errores de deserialización que sean acordes con la logica de negocio ',
                '&bull; Evite renombrar o realizar cambios de tipos sin una transición gradual, esto suele ser un Breaking Change aunque no lo parezca ',
                '&bull; No realice cambios sin planificación, el nivel de planificación debe ser acorde a la magnitud del cambio ',
                '&bull; Usar campos de union, permitiendo que reciba multiples tipos si se tienen planeado evolucionar campos especificos ',
                '&bull; Los nuevos campos deberian ser opcionales, permitiendo que nuevos consumidores no dependan de ellos y viejos consumidores puedan ignorarlos ',
                '',
                '<strong>Cambios comunes y posibles impactos</strong>',
                '&brvbar; -------------- &brvbar; ---------------------------- &brvbar; ---------------------------- &brvbar; --------------------------------- &brvbar; --------------------------------- &brvbar;',
                '&brvbar; <strong>Compatibilidad</strong> &brvbar; <strong>Agregar un campo</strong>             &brvbar; <strong>Eliminar un campo</strong>            &brvbar; <strong>viejos consumidores, nuevos datos</strong> &brvbar; <strong>nuevos consumidores, viejos datos</strong> &brvbar;',
                '&brvbar; -------------- &brvbar; ---------------------------- &brvbar; ---------------------------- &brvbar; --------------------------------- &brvbar; --------------------------------- &brvbar;',
                '&brvbar; <strong>Backwards</strong>      &brvbar; <em>Permitido</em>, los campos nuevos &brvbar; <em>Denegado</em>, consumidores       &brvbar; <em>Puede fallar si</em> antiguos          &brvbar; <em>Funciona si</em> nuevos consumidores   &brvbar;',
                '&brvbar; (versiones     &brvbar;  suelen tener valores        &brvbar;  antiguos esperan la         &brvbar;  consumidores no                  &brvbar;  manejan los campos faltantes     &brvbar;',
                '&brvbar;  anteriores)   &brvbar;   predeterminados            &brvbar;  existencia del campo        &brvbar;  reconocen campos nuevos          &brvbar;  de forma predeterminada          &brvbar;',
                '&brvbar; -------------- &brvbar; ---------------------------- &brvbar; ---------------------------- &brvbar; --------------------------------- &brvbar; --------------------------------- &brvbar;',
                '&brvbar; <strong>Forwards</strong>       &brvbar; <em>Permitido</em>, consumidores      &brvbar; <em>Permitido</em>, nuevos            &brvbar; <em>Funciona si</em> consumidores antiguos &brvbar; <em>Puede fallar si</em> los nuevos        &brvbar;',
                '&brvbar; (versiones     &brvbar;  antiguos pueden ignorar     &brvbar;  consumidores pueden manejar &brvbar;  ignoran de forma segura          &brvbar;  consumidores requieren campos    &brvbar;',
                '&brvbar;  posteriores)  &brvbar;  lo que no conozcan          &brvbar;  campos faltantes            &brvbar;  campos desconocidos              &brvbar;  que faltan en datos antiguos     &brvbar;',
                '&brvbar; -------------- &brvbar; ---------------------------- &brvbar; ---------------------------- &brvbar; --------------------------------- &brvbar; --------------------------------- &brvbar;',
                '&brvbar; <strong>Full</strong>           &brvbar; <em>Permitido</em>, campos nuevos     &brvbar; <em>Denegado</em>, Eliminar campos    &brvbar; <em>Funciona si</em> consumidores antiguos &brvbar; <em>Funciona si</em> nuevos consumidores   &brvbar;',
                '&brvbar; (total)        &brvbar;  con valores predeterminados &brvbar;  rompe la compatibilidad con &brvbar;  ignoran de forma segura          &brvbar;  completan los campos faltantes   &brvbar;',
                '&brvbar;                &brvbar;  para cualquier consumidor   &brvbar;  versiones anteriores        &brvbar;  campos desconocidos              &brvbar;  con valores predeterminados      &brvbar;',
                '&brvbar; -------------- &brvbar; ---------------------------- &brvbar; ---------------------------- &brvbar; --------------------------------- &brvbar; --------------------------------- &brvbar;',
                '',
                '<strong>Patrones y estrategias para una evolución sin tiempo de inactividad</strong>',
                '',
                '<strong>&bull; Patron Expansión-Contracción:</strong>',
                '<strong>- Paso 1 (Expansión):</strong> Agregar nuevos campos al esquema sin remover o cambiar nada ',
                '<strong>- Paso 2 (Adopción):</strong> Actualice los consumidores para leer los nuevos campos y que todos esten actualizados antes del cambio ',
                '<strong>- Paso 3 (Contracción):</strong> Remueva o renombre los campos solo despues de que todos los consumidores esten sincronizados ',
                '',
                '<strong>&bull; Registro de esquemas:</strong>',
                '- Es la autoridad central para las versiones de esquemas, se puede considerar un control de versiones para las estructuras de datos ',
                '- Esto permita que los cambios puedan ser validados, permitidos o rechazados de manera automatica, ayudando a la automatización y gobernanza de los datos ',
            ]),
        new Item(Keys.VERSIONAMIENTO_CHANGELOG,
            [
                '- Un registro de cambios es un archivo que contiene cronológicamente una lista ordenada de los cambios más destacados de cada versión ',
                '- Los changelogs existen para simplificar la comunicación a los usuarios y colaboradores de los cambios importantes que se han realizado en el proyecto ',
                '- Esta convención utiliza <em>Versionado Semántico (SemVer)</em> para nombrar versiones y <em>formato ISO-8601 (YYYY-MM-DD)</em> para fechas ',
                '',
                '<strong>Directrices:</strong>',
                '- Están hechos para ser leídos y entendidos por humanos ',
                '- Cada versión debe tener una entrada ',
                '- Los mismos tipos de cambios deben ser agrupados ',
                '- La última versión debe aparecer de primeras, para que sea cronológico ',
                '- Cada versión debe tener su fecha de lanzamiento ',
                '- No deben incluir mensajes o diffs de los commits a menos que sean realmente relevantes ',
                '- Incluir avisos de breaking changes y nivel de retrocompatibilidad ',
                '',
                '<strong>Tipos de cambio:</strong>',
                '<em>Added:</em> Para funcionalidades nuevas ',
                '<em>Changed:</em> Cambios en las funcionalidades existentes ',
                '<em>Deprecated:</em> Indica las funcionalidades o características obsoletas, que se eliminarán en próximas versiones ',
                '<em>Removed:</em> Características en desuso que se eliminan en esta versión ',
                '<em>Fixed:</em> Corrección de errores ',
                '<em>Security:</em> En caso de vulnerabilidades y posibles efectos colaterales ',
                '',
                '<strong>Cómo mantenerlo:</strong>',
                '- Mantener una sección con el nombre de <em>[Unreleased]</em> es útil para hacer seguimiento de los próximos cambios, ',
                ' - al lanzar la nueva versión el contenido de esta sección será promovido de manera oficial ',
                '- Escoger un estándar y seguirlo, ya sean los sugeridos o cualquier otro que sea conveniente, generando un documento fiable y homogéneo ',
                '- Los enlaces directos a tags, PR, issues, y documentación deben mantenerse actualizados ',
                '- No existe un estándar único, pero una buena convención es <em>Keep a Changelog</em> ',
            ])
    ];
    ...
    [
        new Item(Keys.REFACTORING_CLEAN_CODE,
            [
                ...
                '- Tiene poca complejidad y menos elementos que mantener, un codigo conciso y corto es menos propenso a tener bugs y defectos ',
                ...
                '- Los tests pasan de forma consistente, mantienen una cobertura adecuada y validan los comportamientos esperados del sistema ',
            ]),
        new Item(Keys.REFACTORING_ERRORS_ON_CLEAN_CODE,
            [
                '- Seguir las buenas practicas de manera estricta, tambien puede generar problemas y lo que empieza como una buena intención genera daños a largo plazo ',
                '- “<em>El camino del infierno está empedrado de buenas intenciones</em>” <strong>San Bernardo de Claraval</strong>',
                '',
                '<strong>Una función debe tener una sola responsabilidad:</strong> Valido teoricamente pero no siempre es ideal su aplicación rigurosa ',
                '- El verdadero objetivo es mantener la cohesión más que la fragmentación, mucha division suele aumentar la indirección y dispersa el contexto y su logica ',
                '',
                '<strong>Evitar los comentarios:</strong> Los comentarios aportan valor real cuando documentan el <em>porqué</em> de una decision, no el <em>como</em> que ya expresa el codigo ',
                '- El codigo describe el como, pero los comentarios preservan el razonamiento y las decisiones de diseño que no son evidentes en la implementación ',
                '',
                '<strong>Variables demasiado descriptivas:</strong> Los nombres son muy importantes a la hora de entender el codigo ',
                '- Un nombre excesivamente largo dificulta la lectura del codigo, los nombres deben comunicar la intención y ser concisos en vez de descriptivos en exceso ',
                '',
                '<strong>DRY llevado al extremo:</strong> No repetir codigo es muy importante a la hora de mantenerlo, pero una abstracción prematura, puede ser peor ',
                '- <em>Prefer duplication over the wrong abstraction</em>, <strong>Sandi Metz</strong>; Una buena abstracción suele surgir despues de que la repetición indica el patrón ',
                '',
                '<strong>Probar cobertura y no comportamiento:</strong> Una cobertura del 100% puede parecer una garantia de calidad, pero se pierde el foco real: generar confianza ',
                '- Las pruebas que estan muy acopladas a su implementación y que fallan con cada refactorización, son un lastre y no aportan confianza ',
                '',
                '<strong>Escribir codigo para el futuro hipotetico:</strong> Diseñar anticipando extensibilidad solo desperdicia tiempo, en especial cuando ese futuro nunca llega ',
                '  - Arquitecturas complejas para sistemas que nunca escalan   - Interfaces con una sola implementación',
                '  - Clases abstractas con un solo hijo                        - Patrones que solo agregan complejidad',
                '- Si el cambio llega, es probable que la abstracción anticipada tampoco sea la ideal y deba ser reemplazada de todas formas ',
                '',
                '<strong>Excepciones como flujos de control:</strong> Las excepciones solo deberian lanzarse en errores o casos excepcionales ',
                '- Usarlas para controlar flujo normal, como validaciones o ramas condicionales, encarece el rendimiento y oculta la intención del codigo ',
                '',
                '- Los principios del codigo limpio son guías heurísticas, no mandamientos; su valor está en comprenderlos, no en aplicarlos de forma estricta ',
                '- Comprender el razonamiento detrás de cada principio permite aplicarlos con criterio y reconocer cuándo una excepción está justificada ',
            ]),
        ...
        new Item(Keys.REFACTORING_LEGACY_CODE_I,
            [
        ...
        new Item(Keys.REFACTORING_LEGACY_CODE_II,
            [
                '- Trabajar con codigo antiguo o heredado es inevitable y suele desembocar en varias situaciones al verlo, como: ',
                '',
                '&bull; Codigo raro que es dificil de entender y sin proposito evidente, pero aun asi ha sobrevivido con el tiempo (Olafur’s yellow box)',
                '&bull; Codigo dificil de entender pero que tiene sentido y proposito aunque no lo parezca a primera vista (Chesterton’s Fence)',
                '',
                '<strong>Chesterton’s Fence (La valla de chesterton):</strong> Durante el entendimiento es posible encontrar obstaculos y vallas, no las derribes ',
                ' - o modifiques sin saber porque se pusieron en primer lugar, que este ahi significa que hubo una razon en primer lugar para esa decisión, ',
                ' - estas suelen impedir condiciones o proteger estados que en su momento fueron errores ',
                '',
                '<strong>Olafur’s yellow box (La caja amarilla de Ólafur):</strong> En un puerto normal de Islandia hay una caja amarilla pintanda por ',
                ' - un hombre llamado Ólafur, esa acción sin justificación se volvio costumbre y se siguio manteniendo sin saber el porque ',
                ' - Si el codigo carece de proposito aparente, se convierte en una caja pintada de amarillo que resalta sin alguna utilidad ',
                '',
                '<strong>En el codigo:</strong> Las vallas de chesterton resuelven problemas, pero si no, se pueden considerar cajas amarillas ',
                '',
                '<strong>Estrategias para buscar el significado antes de refactorizar:</strong>',
                '',
                '<strong>Git Blame:</strong> Con el control de versiones es posible retroceder en el tiempo y averiguar porque fueron introducidas esas lineas ',
                '- Además de detalles y contexto importantes para entender la decisión, PR asociados, tickets, autor, despliegue (release), etc.. ',
                '',
                '<strong>Confiar en las pruebas:</strong> Eliminar o refactorizar el codigo y deja que las pruebas que fallen ayuden a diagnosticar que sucedia ',
                '- Este enfoque es ideal solo si la cobertura de pruebas es extensa y cubre el codigo modificado, además de que debe ser hecho de manera controlada ',
                '',
                '<strong>Generar nuevas pruebas:</strong> Aumentar la cobertura para contemplar el comportamiento actual, sea correcto o no ',
                '- De esta manera se generara una idea de que hacia el codigo ',
                '',
                '<strong>Arriesgarse o ignorarlo:</strong> Son los 2 enfoques menos recomendados debido a que el primero implica modificaciónes irresponsables ',
                ' - que pueden introducir bugs/problemas y el segundo implica desconocimiento total o parcial y obliga a dejar el codigo tal cual como esta ',
                ' - volviendo al circulo vicioso en proximas iteraciones, en pro de mantener el codigo funcional y estable ',
            ]),
        new Item(Keys.REFACTORING_MANAGE_DEUDA_TECNICA,
            [
                ...
                '<strong>- Procesos manuales:</strong> Tareas que pueden automatizarse siguen siendo manuales, además no existen procesos de <em>CI/CD</em>',
...
export const CI_CD_INFRAESTRUCTURE =
    [
        ...
        new Item(Keys.CD_CI_DEPLOYMENT_PIPELINES,
            [
                '- En todo proceso de <em>CI/CD</em> siempre tiene una serie de etapas, algunos procesos cuentan con más ó menos que otros segun la necesidad ',
                '',
        ...
        new Item(Keys.CD_CI_MONITORING_AND_ALERTS,
            [
                '- Monitorear es una forma crucial de mantener cualquier sistema funcionando de manera adecuada y sin problemas importantes ',
                ' - debido a la toma de acciones reactivas, preventivas y/o automatizadas que las alertas y el monitoreo brindan ante degradaciones o incidentes ',
                '- Toda esta telemetria suele apegarse al marco y el ecosistema de OpenTelemetry (Otel), aunque puedan haber diferencias de acuerdo al proveedor ',
                '- La monitorización y las alertas son procesos interconectados utilizados para idenficar y mitigar problemas ',
                '- Estas alertas se notifican tan pronto como suceden las novedades, se superan umbrales establecidos o cambios en el sistema ',
                '',
                '- Estos sistemas deben ser fiables, consistentes, resilientes, con alta disponibilidad y capaces de manejar grandes cantidades de datos ',
                '- Esta recopilación de datos no solo ayuda a la detección de problemas, si no que ayuda a crear tendencias y predecir problemas ',
                ' - en base a datos historicos o analisis estadisticos ',
                '- Los sistemas de monitoreo, tambien deben ser monitoreados aunque parezca algo ironico ',
                '',
                '<strong>Observabilidad:</strong> Agregar observabilidad (metricas, logs, traces) a un proyecto puede hacerse de 2 maneras principalmente ',
                '&bull; Implementación personalizada, que requieren una modificación directa en el codigo fuente, y medir lo que se quiere con ayuda de un SDK/API',
                '&bull; Instrumentación, donde no se modifica ningun aspecto del codigo fuente, si no que se agrega un agente que ya lo hace todo ',
                '- En la practica se suelen combinar ambas opciones ya que las metricas generales no siempre pueden observar los datos relevantes para el negocio ',
                '',
                '<strong>Funcionamiento general del monitoreo</strong>',
                '<strong>&bull; Recopilación de datos:</strong> Recolectar y almacenar los datos para convertirlos en metricas y ser enviados al sistema de monitoreo ',
                '<strong>&bull; Analisis de datos:</strong> Los datos se pueden convertir en información valiosa, como patrones, tendencias o detección de anomalias ',
                '<strong>&bull; Visualización:</strong> La información debe ser compartida y contextualizada en tiempo real mediante tableros o paneles para una mejor comprensión ',
                '<strong>&bull; Monitoreo:</strong> Es el proceso continuo que garantiza la vigilancia constante y en tiempo real de los sistemas ',
                '',
                '<strong>Funcionamiento de las alertas</strong>',
                '<strong>&bull; Condiciones de activación:</strong> Conociendo los valores base sobre los que trabaja un sistema, y su importancia para el negocio, ',
                ' - se establecen eventos relevantes, cambios importantes, que sirven como punto de partida para determinar que se considera anormal y su umbral ',
                '<strong>&bull; Generación de alertas:</strong> Cuando se cumplen las condiciones de activación suele ser significado de problemas o anomalias, entonces ',
                ' - se envian las respectivas notificaciones al equipo para evaluar el problema ',
                '<strong>&bull; Notificaciones:</strong> Estas se envian por los distintos medios por los medios que hayan sido configurados, SMS, correos, notificaciones push, ',
                ' - todo esto debe ser personalizado de manera previa, teniendo en cuenta el nivel de gravedad del incidente ',
                '<strong>&bull; Respuesta:</strong> De acuerdo a la alerta recibida es necesario definir que medidad se tomaran, pueden ser inmediatas o posteriores ',
                '',
                '<strong>Stacks conocidos</strong>',
                '&bull; Ingestión y recolección: OpenTelemetry, Edge Delta',
                '&bull; Almacenamiento y búsqueda: ELK, Splunk',
                '&bull; Visualización y alertas: Grafana, Dynatrace, NewRelic, Prometheus',
                '',
                '- Es importante saber que se alertara ya que esto puede desencadenar en una fatiga de alertas y que verdaderas alertas sean ignoradas, ',
                ' - esta fatiga se puede dar por diversas razones, pero generalmente sucede por umbrales mal definidos o falsos positivos sin control ',
                '- Todas las alertas y metricas que se creen deben estar encaminadas al cumplimiento de algun objetivo, de lo contrario solo generan costos y ruido ',
                '- Cuando se solucionan incidente graves es importante documentar que acciones se hicieron, de manera que este conocimiento <em>Postmortem</em> y ',
                ' - <em>análisis de causas raíz</em> (RCA) sea util y ayude a fortalecer el sistema o en su defecto se convierta en un riesgo conocido ',
                '- Todo esto forma un flujo continuo de información sobre el estado y rendimiento de cualquier sistema, siendo fundamental para <strong>SRE</strong> ',
            ]),
        ...
        new Item(Keys.CD_CI_DI_RPO_RTO,
            [
                '- Además de los objetivos de disponibilidad y resiliencia que se tengan, para garantizar la continuidad del negocio ',
                '- Debe existir un plan de recuperación de desastres, para recuperar la carga de trabajo en caso de que se produzca un desastre ',
                '- Se deben centrar en objetivos puntuales de acuerdo a cada caso, como desastres naturales, errores tecnicos ó amenazas humanas ',
                '- Es distinto de la disponibilidad o de la resiliencia ante errores comunes y/o previsibles ',
                '- Tanto el plan de recuperación, como los objetivos RPO y RTO, determinan el nivel de tolerancia al riesgo que la organización está dispuesta a aceptar ',
                '',
                ...
                '- La recuperación de los datos y el tiempo, van directamente relacionados con el metodo de recuperación y replicación (backup, snapshots, journals, etc )',
                '',
                ...
                '- Una forma de lograr un bajo RTO es una arquitectura de alta disponibilidad y redundante, incluso sistemas replicados si se requiere un RTO muy bajo ',
                '',
                ...
                '',
                '<strong>Costo:</strong> Entre mayor sea el objetivo, mayor sera el costo, desde lo economico, hasta lo humano ',
                ...
                '',
                '<strong>Automatización:</strong> Entre mayor sea su alcance, más complejo sera de lograr, por eso suele considerarse IaC como apoyo ',
        ...
        new Item(Keys.CD_CI_ROLLING_DEPLOYMENT,
            [
                '- El rolling deployment es una estrategia de lanzamiento (release) que se usa para implementar cambios de forma incremental y gradual ',
                '- Esto se logra de manera gradual, no se actualizan todas las instancias a la vez, logrando que siempre exista al menos una instancia en funcionamiento ',
                '- El objetivo principal de esta estrategia de despliegue, es lograr un <em>Zero-downtime deployment</em> si se hace correctamente ',
                '',
                '<strong>Preparación:</strong> La nueva versión se prepara y valida en un entorno de pruebas para garantizar que esté lista para producción ',
                '<strong>Actualización:</strong> Primero se actualiza un grupo o un pod, el cual previamente se retiro para no recibir trafico durante la actualización ',
                '<strong>Comprobaciones:</strong> Verificar el estado de la actualización como análisis de métricas, pruebas automatizadas, logs, etc. ',
                '<strong>Balanceador de carga:</strong> Si las instancias fueron correctamente actualizadas y estan listas, vuelven a ser candidatas para manejar trafico ',
                '<strong>Repetición:</strong> El proceso se repite con cada subconjunto de instancias, hasta que todas se encuentren debidamente actualizadas ',
                '',
                '- Kubernetes usa esta estrategia de manera integrada, por lo que muchos de estos pasos ya se encuentran automatizados ',
                '- Esto también va de la mano con el escalado y manejo de recursos integrados, reduciendo/aumentando según la política definida ',
                '- Este tipo de despliegues debe contar con metricas definidas para el exito o fracaso, como tiempo de estabilización, tasa de errores, latencia, etc. ',
                '- Un tema importante a validar con cada versión, es que tan retro-compatible es, pueden existir versiones con Breaking-changes ',
                ' - y de ser asi se debe tener un plan detallado tanto para la actualización como para la reversión de ser necesaria ',
                '',
                '<strong>Pros</strong>',
                '- Reducen el tiempo de inactividad, se garantiza que al menos un grupo de instancias siempre esté en ejecución y disponible ',
                '- La actualización gradual permite que si se encuentra algun error o problema, se puedan revertir los cambios sin afectar a toda la base de usuarios ',
                '- Monitoreo activo de manera que ante cualquier problema o alerta, se puedan tomar medidas correctivas y/o preventivas ',
                '- Implementación continua alineada con metodologias agiles y principios DevOps, primando actualizaciones frecuentes y reducidas ',
                '',
                '<strong>Contras</strong>',
                '- Reversión compleja debido a que se debe hacer sobre elementos o grupos de elementos, y no como un todo a la misma vez ',
                '- Mayor tiempo para el cambio debido a que se hace de manera incremental y gradual ',
                '- Posibilidad de estados inconsistentes debido a la coexistencia temporal de múltiples versiones, especialmente si, ',
                ' - hay cambios en esquemas de datos o contratos de API entre versiones ',
                '- El balanceador de carga puede convertirse en un <em>SPOF</em> si no está debidamente replicado o distribuido, especialmente a gran escala ',
            ])
    ];