...
export const NUBE =
    [
        ...
        new Item(Keys.NUBE_CAAS_DAAS_FAAS,
            [
        ...
        new Item(Keys.NUBE_12_FACTORES,
            [
        ...
        new Item(Keys.NUBE_DNS,
            [
                ...
                '',
                '<strong>¿ Que ocurre sin fallan ?</strong>',
                '- Si un servidor DNS no responde o incluso falla, el proceso de resolución de nombres no puede completarse, esto provoca interrupciones en los servicios ',
                '- Existen formas de mitigar este riesgo como infraestructura multi-zona o redundancia mediante replicas primarias-segundarias ',
            ]),
        ...
        new Item(Keys.NUBE_TWO_PHASE_COMMIT,
            [
                '- En la computación distribuida, uno de los problemas más importantes es lograr un consenso entre multiples nodos, lo cual no es facil de lograr ',
        ...
        new Item(Keys.NUBE_DISPONIBILITY,
            [
                ...
                '- Una arquitectura madura que permita CI/CD          &brvbar;            <strong>Nivel</strong>              &brvbar;    <strong>%</strong>     &brvbar;   <strong>Tiempo</strong>   &brvbar;',
        ...
        new Item(Keys.NUBE_PACELC,
            [
                ...
                '',
                '&bull; Este teorema cubre aspectos faltantes de CAP y establece que un sistema que replique datos: ',
                '- <strong>P</strong>    Si maneja <em>Particiones</em>',
                '- <strong>A/C</strong>  Si hay una Partición un sistema distribuido puede equilibrar entre Disponibilidad(<strong>A</strong>) y Consistencia(<strong>C</strong>)',
                '- <strong>E</strong>    Sino ',
                '- <strong>L/C</strong>  Si no hay particiónes, el sistema se equilibrara entre Latencia(<strong>L</strong>) y Consistencia(<strong>C</strong>)',
                '',
                '&bull; Esto es importante porque ayuda a pensar en incognitas y problemas que se deben tener en cuenta a la hora de diseñar ',
                '- ¿ Un enpoint API debería fallar rápidamente o proporcionar datos obsoletos ?',
                '- ¿ Debería aplicarse una coherencia estricta a nivel general ?',
                '- ¿ Debemos priorizar la latencia o la precisión ?',
                '- ¿ Debemos sacrificar el rendimiento por garantías transaccionales ?'
            ]),
    ...
    [
        ...
        new Item(Keys.APIS_STANDARS,
            [
                ...
                '<strong> - Integrar DevOps y CI/CD</strong> Reduciendo el tiempo entre etapas y agilizando el depliegue y mantenimiento con cada versión ',
        ...
        new Item(Keys.APIS_REST_VS_MESSAGING,
            [
                ...
                '- En el pasado los <em>Enterprise Service Buses</em> dominaban junto a <strong>SOA</strong> donde era comun integrar la lógica de integración ',
                ...
                '<strong>- Resiliencia y Errores:</strong> Al tener plataformas que garantizan la entrega de mensajes, se reduce la lógica al no tener que lidiar con mensajes perdidos ',
        ...
        new Item(Keys.APIS_API_FIRST_VS_CODE_FIRST,
            [
                ...
                '- Este enfoque es el más tradicional en el cual ya existe la implementación de la lógica que expondra el API ',
                '',
                '<strong>Ventajas</strong>',
        ...
        new Item(Keys.APIS_SECURITY,
            [
                '- Las APIs suelen ser los puertos de entrada principales de las aplicaciones, si no se protegen adecuadamente los atacantes lo aprovecharan ',
                '- Las grandes empresas lo entienden y por eso suelen gastar tiempo y dinero para lograrlo, acontinución se presentan 7 tecnicas basicas para lograrlo ',
                '',
                '<strong>Rate Limiting:</strong> Controlar la cantidad de solicitudes que un cliente puede realizar en un tiempo determinado ',
                '- Previene ataques de fuerza bruta e intentos de DDoS, se puede aplicar por IP, Usuario, Endpoint, o global, e incluso ser adaptable ',
                '',
                '<strong>CORS (Cross-Origin Resource Sharing):</strong> Restringe los dominios que pueden llamar a tu API ',
                '- Sin esta función sitios web maliciosos, pueden engañar al navegador del usuario conectado, para enviar solicitudes no autorizadas ',
                '- Esto no protege contra clientes maliciosos, que suelen ser ataques directamente contra tu API ',
                '',
                '<strong>Inyección SQL y NoSQL:</strong> Las entradas no desinfectadas pueden permitir que los atacantes lean, modifiquen o eliminen los datos ',
                '- Para evitar esto siempre se deben usar consultas parametrizadas o ayudarse de los ORM ',
                '- Tambien se deben restringir los permisos en la medida de lo posible, siempre y cuando no afecten la operación de las aplicaciones ',
                '',
                '<strong>Firewall:</strong> Filtra el tráfico malicioso antes de que llegue a su aplicación',
                '- Se pueden bloquear peticiones que tengan patrones de inyección SQL o métodos HTTP inusuales ',
                '',
                '<strong>API Gateway:</strong> Al funcionar como único punto de entrada, el funcionamiento del sistema se encuentra parcialmente ofuscado ',
                '- Tambien puede centralizar otros elementos relacionados con la seguridad como logging, autenticación y autorización ',
                '',
                '<strong>VPN (Virtual Private Networks):</strong> Una VPN garantiza que solo el tráfico autenticado y cifrado de rangos de IP confiables pueda acceder a ellos ',
                '- Para entornos públicos es difícil de implementar por lo que suele ser usado en entornos empresariales y B2B ',
                '',
                '<strong>CSRF (Cross-Site Request Forgery):</strong> Engaña al navegador de un usuario conectado para que realice solicitudes no deseadas ',
                '- Se puede solucionar requiriendo el uso de un token y una cookie de sesión ',
                '',
...
export const META_ESTRUCTURAS =
    [
        ...
        new Item(Keys.META_ESTRUCTURAS_LAYERS_SOFTWARE,
            [
        ...
        new Item(Keys.META_ESTRUCTURAS_ESTRUCTURAS_DATOS_II,
            [
                ...
                '- Las ubicaciones estan asignadas de manera secuencial, cualquier ubicación se accede de manera instantanea <strong>Aplicaciones:</strong> ',
                ...
                '- Son una forma segura de procesamiento ordenado, garantizando una gestión secuencial <strong>Aplicaciones:</strong>',
                '  - Atención de usuarios                - Mensajeria                    - Impresiones ',
                '  - Almacenamiento en buffers           - Programación de tareas        - Equilibrio de cargas ',
                '',
                ...
                '- Tambien pueden enlazarse con el elemento anterior si son del tipo doblemente enlazadas, permitiendo recorridos bidireccionales ',
                '- Si se enlaza el inicio con el final, formando un bucle continuo, se le conoce como lista circular <strong>Aplicaciones:</strong>',
                ...
                '',
                '<strong>Trie:</strong> Tambien conocido como arbol de prefijos, es un arbol especializado para almacenar y buscar cadenas de texto, de acuerdo a sus prefijos ',
                '- Cada nodo representa un solo caracter, logrando que cada ruta que se forma desde la raiz forme una palabra <strong>Aplicaciones:</strong>',
                '  - Autocompletar                       - Corrector ortografico         - Enrutamiento de direcciones IP',
                '',
        ...
        new Item(Keys.META_ESTRUCTURAS_FUNCIONES_LAMBDA,
            [
                ...
                '- Comunes en los lenguajes funcionales y/o multiparadigma y permiten desde expresiones simples hasta lógica compleja con multiples instrucciones',
    ...
    [
        ...
        new Item(Keys.MICROSERVICES_BENEFITS,
            [
                ...
                '',
                '<strong>&bull; Enfoque en el dominio, la lógica del negocio</strong>',
        ...
        new Item(Keys.MICROSERVICES_TRADEOFFS,
            [
                ...
                '- Actualmente areas como <em>CI/CD</em> los despliegues suelen ser automatizados y tienen multiples mecanismos de recuperación ',
        ...
        new Item(Keys.MICROSERVICES_PATTERNS,
            [
                '<strong>Patron Saga:</strong> Proporciona una gestión eficaz de la transacción, usando una secuencia de transacciones locales ',
                ' - de manera que todas las operaciones se puedan ejecutar, o se realicen acciones de compensación, para deshacer los cambios anteriores ',
                ...
                '- Los eventos de compensación deben ser lo suficientemente generales para ser reutilizables y generalizables, ',
                ' - manteniendo la capacidad de adaptación para acciones especificas que requiera el negocio ',
                '',
                '&bull; <strong>Coreografia:</strong> Cada microservicio produce y escucha, y se comunican a traves de un canal en comun ',
                '- Es más facil de implementar pero se puede volver desordenado entre más flujos y operaciones se deban manejar ',
                '- Si no hay una clara visualización de los flujos de trabajo, estas son fragiles y factibles a errores ',
                '&bull; <strong>Orquestación:</strong> Un orquestador es el responsable de tomar las decisiones, y los servicios solo ejecutan ',
                '- Escala mejor gracias al cerebro principal que coordina todo, pero este a su vez puede convertirse en un cuello de botella ',
                '- Sin mecanismos de redundancia no se puede garantizar una alta disponibilidad convirtiéndose en un Unico punto de fallo <strong>SPOF</strong> ',
                '',
                ...
                '- Mantener una sola BD es un beneficio a corto plazo, por lo que se puede optar por que cada servicio tenga una propia ',
                '- Las Bases de datos tambien pueden ser compartidas, con la acotación de que los esquemas y la independencia de los datos deben ser claras ',
                ...
                '- Los servicios no deben conocer bases de datos que no esten bajo su administración, esto agrega acoplamiento y complejidad indeseada entre servicios ',
                '',
                ...
                '- Se debe cuidar la complejidad ya que entre más lógica y llamados de red la latencia y el tiempo de procesamiento tambien creceran ',
                '',
                ...
                '- El manejo de eventos tambien es inherente a la consistencia eventual de los datos ',
                '',
                ...
                '&bull; <strong>Comandos:</strong> Manejan todo lo relacionado con la creación, eliminación y actualización ',
                '&bull; <strong>Consultas:</strong> Maneja las consultas y la obtención de datos ',
                ...
                '- Esto minimiza la cantidad de llamados necesarios entre el cliente y la aplicación además de ser seguro mediante sistemas de aunteticación ',
                '- Facilmente combinable con el patron Backend For Frontend para una mayor adaptación con cada cliente ',
                '',
                ...
                '- Este maneja 3 estados principales, y como si de una maquina de estados se tratara, controla el flujo de la aplicación ',
                '&bull; <strong>Abierto:</strong> La solicitud falla y se devuelve una excepción, cerrando el acceso por un tiempo determinado ',
                '&bull; <strong>Cerrado:</strong> Enruta las solicitudes y cuenta la cantidad de fallas, hasta que no pasen de un umbral ',
                '&bull; <strong>Half-Open:</strong> Solo se permite un pequeño numero de operaciones, cambiando entre estados de acuerdo a si fallan o son exitosas ',
                '',
                '<strong>Bulkhead Isolation:</strong> Aislamiento de recursos de manera que contengan solamente los servicios afectados ',
                '- La adecuada redistribución de recursos permite que todo el sistema siga funcional como un conjunto, aunque existan partes dañadas o con errores ',
            ]),
        ...
        new Item(Keys.MICROSERVICES_PUSH_PULL_EVENTS,
            [
                ...
                '&bull; La lógica de procesamiento es más robusta y tolerante a fallos ',
                '',
                '<strong>Desventajas</strong>',
        ...
        new Item(Keys.MICROSERVICES_DATA_ACCESS_PATTERNS,
            [
                '- El acceso y la manipulación de los datos es un aspecto fundamental de las aplicaciones, y para los microservicios existen diversos enfoques ',
                '',
                '<strong>Antipatrón - Acceso directo a la BD:</strong> Es una acción simple pero es un antipatrón explícito en el que se puede caer ',
                '- Esto implica que el <em>Sistema B</em> consulta directamente la base de datos del <em>Sistema A</em>, la principal razón en contra ',
                ' - es el acoplamiento indebido que se genera, la independencia y el gobierno de los datos se reduce o deja de existir ',
                '- Esto solo debería tenerse en cuenta bajo escenarios específicos que requieran latencia mínima y bajo estrictas reglas de gobierno de datos ',
                '',
                '<strong>Mediante APIs:</strong> Una forma simple y común es que el <em>Sistema A</em> exponga una API síncrona la cual ',
                ' - mediante protocolos (REST, gRPC o GraphQL) recibe la respuesta con los datos solicitados por el <em>Sistema B</em> ',
                '- De manera previa se define un contrato y sus versiones, que especifica la estructura de solicitud y respuesta entre ambos sistemas ',
                '- Si la carga de solicitudes es muy alta se pueden implementar estrategias para su manejo como réplicas de lectura o caches ',
                '- Si el tráfico entre microservicios, o la cantidad de datos necesaria para realizar las operaciones es muy alta, ',
                ' - se vuelve necesario revisar otros enfoques y el nivel de dependencias entre microservicios ',
                '',
                '<strong>ETL & Tareas por lotes:</strong> Existen casos donde los datos no tienen necesidad de inmediatez, o se deben agrupar para análisis ',
                '- Se puede aplicar un enfoque de Extracción, Transformación y Carga, cada vez que sea necesario ',
                '- Esta tarea se puede automatizar y realizarse por lotes, enviando datos de manera periódica ',
                '',
                '<strong>Solicitudes asincrónicas:</strong> La solicitud de datos del <em>Sistema B</em> hacia el <em>Sistema A</em> ',
                ' - se realiza de manera asíncrona, usando un intermediario (colas o topics), después de recibir la solicitud y procesarla ',
                ' - se envía la respuesta mediante el mismo intermediario, esto agrega complejidad operativa a costa de reducir el acoplamiento ',
                '- Generalmente usado cuando los procesos involucrados suelen ser demasiado complejos o demorados evitando bloquear recursos por mucho tiempo ',
                '',
                '<strong>Envío de mensajes (Pub/Sub):</strong> Si se quiere mantener todos los cambios sincronizados y se permite cierta demora ',
                '- Se puede hacer que el <strong>Sistema A</strong> publique los cambios realizados sobre los datos, mediante mensajes en un intermediario ',
                ' - dependiendo de la acción que se haya realizado, el <em>Sistema B</em> puede consumir eventos ó comandos, en linea con CQRS ',
                '- Esta copia de datos permite que solo se replique la información necesaria y de utilidad en otros sistemas ',
                '- Si el <em>Sistema A</em> deja de publicar los mensajes por cualquier razón, los sistemas suscritos empezarán a quedarse desincronizados, ',
                ' - por lo que también se puede usar un camino más directo (CDC), sin necesidad de intervenir en la aplicación ',
                '',
                '<strong>Change Data Capture (CDC):</strong> Este enfoque consume los cambios directamente desde la fuente de la verdad, la BD del <em>Sistema A</em>',
                '- Este flujo de datos puede transformarse y producirse según las necesidades, permitiendo que cada consumidor ',
                ' - pueda usar su propia lógica de negocio y el <em>Sistema B</em> se convierta en algo más que una replicación de datos ',
                '- Es más compleja operativamente que usar APIs, pero también aporta flexibilidad y actualizaciones en tiempo real (Streaming)',
                '',
                '&brvbar;      <strong>Enfoque</strong>       &brvbar;       <strong>Latencia</strong>       &brvbar; <strong>Complejidad</strong> &brvbar; <strong>Acoplamiento</strong> &brvbar;   <strong>Consistencia</strong>   &brvbar;                        <strong>Casos de uso</strong>                         &brvbar;',
                '&brvbar; API REST           &brvbar; Baja (ms)            &brvbar; Baja        &brvbar; Alto         &brvbar; Inmediata        &brvbar; CRUD, Consultas esporádicas                                 &brvbar;',
                '&brvbar; Async (Cola/Topic) &brvbar; Media (seg)          &brvbar; Media       &brvbar; Medio        &brvbar; Eventual         &brvbar; Procesos largos, operaciones no urgentes                    &brvbar;',
                '&brvbar; Pub/Sub            &brvbar; Media-Alta (seg-min) &brvbar; Alta        &brvbar; Bajo         &brvbar; Eventual         &brvbar; Sincronización eventual, notificaciones, eventos de dominio &brvbar;',
                '&brvbar; CDC                &brvbar; Muy Baja (ms)        &brvbar; Muy Alta    &brvbar; Medio-Bajo   &brvbar; Casi tiempo real &brvbar; Streaming, replicación de datos, analítica & BI             &brvbar;',
                '&brvbar; ETL/Lotes          &brvbar; Muy Alta (horas)     &brvbar; Media       &brvbar; Bajo         &brvbar; No crítica       &brvbar; Reportes, análisis, procesos programados, data warehouse    &brvbar;',
            ])
    ];

export const MICRO_SERVICES_ADVANCED =
    [
        new Item(Keys.MICROSERVICES_ADVANCED_BY_DEFAULT,
            [
        ...
        new Item(Keys.MICROSERVICES_ADVANCED_ANTI_PATTERNS,
            [
                ...
                '',
                '<strong>Arquitectura Spageti:</strong> Es una arquitectura sin ninguna organización o división lógica, solamente una maraña de componentes conectados ',
        ...
        new Item(Keys.MICROSERVICES_ADVANCED_PRODUCER_CONSUMER_PROBLEM,
            [
        ...
        new Item(Keys.MICROSERVICES_ADVANCED_IDEMPOTENCY_I,
            [
                '- Manejar eventos inherentemente conlleva la posibilidad de duplicación de mensajes, esto puede causar problemas en los consumidores ',
                '- Es normal que los eventos porten un ID(hash, UUID, etc..), que lo identifique de manera clara, esto con el fin de crear <em>consumidores idempotentes</em> ',
                ...
                '',
                '<strong>Orden:</strong> Es importante para que los eventos sean procesados correctamente, pero los brokers no lo garantizan de manera global, o tienen condiciones especificas ',
                '',
                '<strong>Exactly-Once:</strong> Esta garantía de entrega pareciera la mejor opción, pero su alcance es limitado al broker y tiene un alto costo de coordinación ',
                '',
                ...
                '',
                '<strong>Transacciones:</strong> Inician el momento en que se consume cada mensaje, esto evita inconsistencias pero agrega complejidad e impacta el rendimiento ',
                ' - las transacciones por si solas no evitan la duplicación, deben ser combinadas con procesos de negocio que lo validen ',
                '',
                '<strong>Patron Outbox:</strong> Es una forma de evitar que las transacciones tengan problemas de tipo <em>Dual-Write</em> ',
                '- Este cubre el proceso completo Consumo -> Procesamiento -> Publicación -> Confirmación ',
                '',
                '<strong>Change Data Capture:</strong> Los cambios en los datos serán capturados y publicados en tiempo real mediante un componente externo, ',
                ' - diferente de la aplicación, siendo un proceso general asociado que informa cualquier cambio en la BD <em>(insert, update, delete)</em> ',
                '',
                '<strong>Keys:</strong> Almacenar los ID de cada mensaje consumido, permitiendo detectar y descartar instantáneamente cualquier duplicado ',
                '',
                '<strong>UPSERT:</strong> Finalmente se puede tener en cuenta el enfoque tradicional, es la tecnica más simple y eficiente generalmente si no hay un gran <em>throughput</em> ',
            ]),
        new Item(Keys.MICROSERVICES_ADVANCED_IDEMPOTENCY_II,
            [
                '<strong>¿ Porque se pueden duplicar las operaciones ?</strong>',
                '',
                '<strong>Fallos de red:</strong> La solicitud se envía pero la respuesta se pierde; el cliente, al no tener certeza del resultado, vuelve a intentarlo ',
                '<strong>Acciones del usuario:</strong> Hacer click en el boton varias veces ',
                '<strong>Bloqueos:</strong> El servicio procesa la solicitud pero se bloquea o falla antes de responder ',
                '<strong>Reintentos:</strong> Ya sea por parte de un API o de un consumidor, las solicitudes fallidas pueden volver a procesarse ',
                '<strong>DLQ:</strong> Reenvios de mensajes no confirmados ',
                '',
                '&brvbar;   <strong>Operaciones idempotentes</strong>  &brvbar; <strong>Operaciones NO idempotentes</strong>  &brvbar;',
                '&brvbar; --------------------------- &brvbar; ---------------------------- &brvbar;',
                '&brvbar; GET, Lectura de datos       &brvbar; POST, Creación de datos      &brvbar;',
                '&brvbar; DELETE(204) Eliminación     &brvbar; POST, Operaciones bancarias  &brvbar;',
                '&brvbar; (PUT) Actualizaciones       &brvbar; PATCH, Actualizacion parcial &brvbar;',
                '&brvbar;                             &brvbar; Envio de correos             &brvbar;',
                '',
                '<strong>Estrategias:</strong>',
                '',
                '<strong>Keys:</strong> El cliente genera una llave unica para cada intento, que se envia con cada solicitud, asi los duplicados pueden ser facilmente descartados ',
                '- Almacenarlas consume espacio y por eso pueden tener una politica de borrado de datos(<strong>TTL</strong>) además de que deben estar debidamente optimizados(indices) ',
                '',
                '<strong>Constraints:</strong> Utilizar restricciones de unicidad en BD permite rechazar automáticamente operaciones duplicadas, sin depender de lógica de aplicación ',
                '- No depende de algun almacenamiento externo, pero si requiere el manejo explicito de estas excepciones de casos duplicados o eventos viejos ',
                '',
                '<strong>Bloqueo distribuido:</strong> Usar un almacenamiento externo para adquirir locks con <strong>TTL</strong> sobre una clave única por operación, pueden ser de tipo <strong>clave-valor</strong> ',
                '- Garantiza exclusión mutua ante alta concurrencia, suelen tener latencias muy bajas y son relativamente constantes ',
                '',
                '<strong>Hash:</strong> Crea un hash con valores especificos a partir de los campos/payload del mensaje, si no se detectan cambios se puede tomar la decisión de no hacer nada ',
                '',
                '<strong>Defensa profunda:</strong> Usar multiples estrategias y combinarlas segun la conveniencia es recomendable de acuerdo a la criticidad de la operación ',
            ]),
        new Item(Keys.MICROSERVICES_ADVANCED_CACHE_FAILS,
            [
        ...
        new Item(Keys.MICROSERVICES_ADVANCED_DISTRIBUTED_TRACING,
            [
        ...
        new Item(Keys.MICROSERVICES_ADVANCED_DEAD_LETTER_QUEUE,
            [
        ...
        new Item(Keys.MICROSERVICES_ADVANCED_DISTRIBUTED_TRANSACTIONS,
            [
        ...
        new Item(Keys.MICROSERVICES_ADVANCED_PROBLEMS,
            [
                ...
                '&bull; Una cultura de DevOps, CI/CD, Monitoreo y gestión de incidentes madura y automatizada, para manejar sistemas distribuidos ',
        ...
        new Item(Keys.MICROSERVICES_ADVANCED_ARQUITECTURE_BACK_MONOLITH,
            [
        ...
        new Item(Keys.MICROSERVICES_ADVANCED_DISTRIBUTED_MONOLITH,
            [
                '- Los microservicios son una de las arquitecturas más usadas en la actualidad, la cual resuelve muchos problemas modernos, y agrega otros más tambien ',
                '- Los sistemas distribuidos llevan una gran complejidad inherente que puede desestabilizar sistemas completos si no se controla adecuadamente ',
                '',
                '&bull; Latencia               &bull; Madurez tecnica',
                '&bull; Fallos parciales       &bull; Observabilidad',
                '&bull; Consistencia eventual  &bull; Complejidad operativa',
                '',
                '- Si un sistema distribuido no es correctamente diseñado se puede crear un <strong>Monolito distribuido</strong> que combina lo peor de ambos mundos ',
                '- Toda la complejidad de un sistema distribuido junto al acoplamiento estrecho que ya tiene el monolito, esto se suele dar cuando ',
                ' - se fragmenta un monolito de manera inadecuada, distribuyendolo por la red sin una conversación de diseño ',
                '- Un sintoma de este problema suele ser mantener la misma Base de Datos para todos los <em>nuevos</em> servicios ',
                '',
                '<strong>Desventajas y/o problemas</strong>',
                '- Complejidad exponencial, lo que solia ser una sola operación, ahora requiere de orquestación de multiples componentes ',
                '- Problemas de rendimiento, la latencia se incrementa con llamada por la red, entre más servicios se involucren, peor sera ',
                '- Fallos en cascada, si un servicio falla y no se detiene de manera apropiada, las piezas empezaran a caer como un domino ',
                '- Los despliegues en lugar de ser independientes, ahora requieren una coordinación precisa de diversas partes ',
                '- Las pruebas de integración o de End-to-End se vuelven cada vez más complejas y dificiles de mantener ',
                '- Los problemas ya no se limitan a un solo componente, sin una adecuada observabilidad identificar el componente responsable puede ser una pesadilla ',
                '- Las transacciones distribuidas y la consistencia de los datos se convierte en un reto que obliga a implementar patrones como <em>SAGA</em> o <em>Two Phase Commit</em> ',
                '',
                '<strong>Alternativas y soluciones</strong>',
                '- Realizar un diseño en base a los multiples <strong>dominios</strong> (DDD) existentes, las funcionalidades previas no necesariamente se van a mantener igual ',
                '- <em>Arquitectura Basada en eventos</em> como forma de comunicación asincrona, rompiendo el acoplamiento temporal y que los sistemas sean más independientes ',
                '- Bases de datos independientes para cada servicio, con datos desnormalizados si es necesario ',
                '- Evitar las consultas sincronas en la medida de lo posible, mediante copias de datos o el uso de algun cache ',
                '- Implemente acciones para tener un sistema resiliente, la resiliencia no significa 0 fallos, significa no detenerse por completo ',
                '- Un monolito modular puede ser la mejor opción, especialmente para equipos pequeños ',
                '',
                '<strong>¿ Es necesario romper un monolito ?</strong>',
                '- Para saber la respuesta correcta es ideal poder responder estas otras primero ',
                '&bull; ¿ Tiene problemas reales que sólo los microservicios pueden resolver ?',
                '&bull; ¿ Tiene su equipo la madurez para manejar sistemas distribuidos, o al menos cuenta con capacidades de SRE y DevOps ?',
                '&bull; ¿ Ha definido límites claros en función del dominio del negocio ?',
                '&bull; ¿ Tiene suficiente capacidad de observación para depurar sistemas distribuidos ?',
                '&bull; ¿ Pueden sus servicios fallar independientemente sin afectar a todo el sistema ?',
                '&bull; ¿ Tiene herramientas de CI/CD y estrategias de recuperación integradas en la operación cotidiana ?',
                '&bull; ¿ Existen métricas claras de negocio que justifiquen la migración más allá de lo técnico ?',
                '&bull; ¿ El costo de infraestructura adicional requerido, es sostenible a largo plazo ?',
                '&bull; ¿ Hay un plan de gobernanza para manejar versiones de APIs, contratos y dependencias entre servicios ?',
                '&bull; ¿ El equipo está preparado para manejar la carga cultural y organizacional que implican los microservicios (autonomía, comunicación, ownership) ?',
                '',
                '- Si alguna o varias de esas preguntas son un <strong>NO</strong> intente mejorar esa falencia y luego vuelva a considerar la migración (StranglerFig app)',
            ]),
        new Item(Keys.MICROSERVICES_ADVANCED_DISTRIBUTED_LOCKING,
            [
                '- En entornos con múltiples instancias de un mismo microservicio desplegadas, cuando existen operaciones de una sola vez que no admiten duplicados ',
                '- Se necesita una forma de compartir el estado y coordinar la ejecución de tareas de forma unica, o de lo contrario la concurrencia afectaria el resultado ',
                '',
                '<strong>Reglas de un bloqueo distribuido:</strong>',
                '<strong>- Seguridad (Exclusión Mutua):</strong> Solo un cliente puede tener bloqueado el proceso al mismo tiempo ',
                '<strong>- Interbloqueo (No-deadlock):</strong> Si un cliente falla o se desconecta otro deberia poder adquirir el bloqueo eventualmente ',
                '<strong>- Consistencia y Tolerancia a fallos:</strong> El sistema debe comportarse de forma predecible, ante fallos de red o fallos en los nodos ',
                '<strong>- Tiempo de vida:</strong> (<strong>T</strong>ime <strong>T</strong>o <strong>L</strong>ive) En caso de fallos durante o antes de la liberación, estas deben borrarse automaticamente despues de <strong>X</strong> tiempo ',
                '',
                '<strong>Formas de bloqueos:</strong>',
                '<strong>&bull; Basados en base de datos:</strong> La forma más sencilla y con garantias transaccionales <strong>ACID</strong> ',
                '- No están diseñados para escenarios de mucha concurrencia ni condiciones de carrera frecuentes ',
                '<strong>&bull; Bloqueos con sistemas externos (Redis):</strong> Un almacenamiento clave-valor ampliamente usado para sistemas de cache, sesiones y bloqueos distribuidos ',
                '- Cada vez que un cliente solicita un bloqueo, se crea si no existe, y si otro cliente lo solicita, se le negará hasta que expire el <strong>TTL</strong> o se libere ',
                '<strong>&bull; Algoritmo Redlock (multi-Redis):</strong> Con múltiples instancias se deben ejecutar pasos extras para que todas las instancias esten debidamente sincronizadas ',
                '- No utiliza <em>protocolos de consenso</em>, por lo que si hay desfases en los relojes o latencias elevedas, retrasara el bloqueo o provocara errores de exclusión ',
                '<strong>&bull; Bloqueos ZooKeeper:</strong> Esta herramienta fue creada explícitamente para este proposito y es independiente de los relojes de cada cliente ',
                '- Es fuertemente consistente y usado por sistemas críticos, a costa de ser más complejo y lento que los usados por Redis ',
                '',
                '- Para evitar problemas graves si algún proceso falla durante su ejecución, además de usar tiempos razonables para los bloqueos, ',
                ' - también se usan <em>Fencing Tokens</em> de manera que aumente algun contador/secuencia con cada bloqueo y solo se permita ejecutar al más reciente ',
                '- La monitorización de los bloqueos ayuda a prevenir errores y al diagnostico de posibles comportamientos anómalos, ',
                ' - como el retraso en la obtención, ejecuciones más largas que el TTL, o mas de un retenedor simultaneo ',
            ]),
        new Item(Keys.MICROSERVICES_ADVANCED_PROCESSING_ARCHETYPES,
            [
                '- En los sistemas distribuidos la mensajería asíncrona es un tema estructural implícito ',
                '- Los topics, las colas y los streams son formas de consumo diferentes que involucran ventajas, problemas y diversas formas en las que pueden fallar ',
                '- Los microservicios generalmente cuentan con una o más formas y también suelen combinarlas de acuerdo a sus necesidades ',
                '',
                '<strong>NOOP (No Operation):</strong> Un caso simple donde se reciben los mensajes pero no se hace ninguna acción con ellos ',
                '- Aunque parezca un caso irrelevante puede suceder en la práctica si el consumidor determina que el mensaje no debe ser procesado ',
                '- No importa si el consumidor se retrasa o falla, el resultado seguirá teniendo un impacto nulo ',
                '',
                '<strong>Calcular valores, publicar el resultado:</strong> La complejidad aumenta ya que se realiza alguna acción con el mensaje o sobre el mensaje ',
                '- Esto implica transformaciones de datos, cálculos de valores históricos, persistencia/actualización de datos, llamados a APIs externas, etc. ',
                '- Es importante considerar si la operación debe ser idempotente, necesitando la gestión adecuada de reintentos/fallos ',
                '',
                '<strong>Consultar datos adicionales, publicar el resultado:</strong> Si hacen falta datos adicionales para su procesamiento ',
                '- Se pueden consultar sistemas externos como una API o una base de datos, similar al caso anterior, agregando la dependencia de un sistema externo ',
                '- Estas operaciones se vuelven sensibles a la latencia y tienen un mayor acoplamiento con los sistemas externos ',
                '',
                '<strong>ACK & Confirmación de procesamiento:</strong> Confirmar los mensajes con el broker solo debería suceder después de que el proceso se complete ',
                '- Debido a que los fallos significan reprocesamiento, cualquier mutación del estado debe ser idempotente ',
                '- Todos los brokers permiten configurar el tiempo de retención (TTL), después del cual los mensajes son eliminados automáticamente ',
                '- Reintentar es una forma de resiliencia, pero una mala configuración se puede convertir en una fuente de problemas para la consistencia o el rendimiento ',
                '',
                '<strong>Logs & Monitoreo:</strong> Comprender el estado del sistema se facilita si se tiene una forma de logging estructurada ',
                ' - tanto para mensajes exitosos como fallidos, además de tener información relevante para generar alarmas o seguir incidentes específicos ',
            ]),
        new Item(Keys.MICROSERVICES_ADVANCED_SPLIT_BRAIN,
            [
                '- La consistencia de los datos en sistemas distribuidos es un reto y si no se maneja correctamente se pueden presentar problemas ',
                '- Cuando una partición de red o fallo de comunicación divide un cluster en grupos aislados, se produce un split-brain ',
                '- Se crean grupos que operan de forma independiente, generando inconsistencias y conflictos en los datos del sistema ',
                '',
                '<strong>Técnicas para evitar el split-brain:</strong>',
                '<strong>Basadas en Quorum:</strong> Estos métodos requieren que la mayoría de los nodos acuerden operaciones como actualizaciones o cambios de estado ',
                '- Ideal para sistemas donde la consistencia es crucial, como en sistemas bancarios ',
                '',
                '<strong>Algoritmos de consenso:</strong> <em>Paxos</em> y <em>Raft</em> son protocolos de consenso diseñados para mantener la consistencia ante fallos de nodos y particiones de red ',
                '- Estos algoritmos son ideales para bases de datos distribuidas, sistemas de archivos y otros sistemas que requieren garantías y una consistencia fuerte ',
                '',
                '<strong>Healthcheck:</strong> Es un mecanismo usado por herramientas de monitoreo, revisando constantemente la disponibilidad de todos los nodos ',
                '- Si algun nodo no responde al chequeo, el servicio puede emitir alertas o marcarse como disponible ',
                '',
                '<strong>Heartbeat:</strong> Es una señal periódica que cada nodo emite; si no se recibe dentro del timeout establecido, se declara como un fallo ',
                '- En este caso las réplicas se marcan solo para lectura y se evita la escritura o operaciones que requieran de consenso ',
                '',
                '- En algunos casos es tolerable, como en sistemas con <strong>consistencia eventual</strong> que están diseñados para tolerar inconsistencias temporales entre réplicas, ',
                ' - pueden ser sistemas colaborativos, redes sociales, y caches, donde se prioriza la disponibilidad sobre la consistencia (CAP: AP, PACELC: PA/EL) ',
            ])
    ];
