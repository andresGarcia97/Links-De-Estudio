    ...
    [
        ...
        new Item(Keys.TESTING_FIRST,
            [
                ...
                '<strong>Timely:</strong>          Toda prueba debe ser oportuna y debe tener una razón de ser, si no se pueden justificar, estas habran sido una perdida de tiempo ',
            ]),
        ...
        new Item(Keys.TESTING_AAA_GWT,
            [
                '- Crear pruebas no solo se trata de detectar errores, sino que sean claras y eficientes, esto se logra mediante patrones reconocidos ',
                '- Una batería de pruebas coherentes es fácil de entender y de mantener, mas aún si se adhieren a alguno de los siguientes patrones ',
                '',
                '<strong>AAA:</strong> Ideal para pruebas unitarias o de baja complejidad ',
                '<strong>Arrange:</strong> Se organiza todo el contexto necesario y se deben establecer las condiciones del test ',
                '<strong>Act:    </strong> Es la ejecución del fragmento a testear, invocando al método en cuestión ',
                '<strong>Assert: </strong> Último paso de la prueba y es la comprobación de los resultados o comportamientos esperados ',
                '',
                '<strong>GWT:</strong> Ideal para pruebas de alto nivel o que validen comportamientos más avanzados ',
                '<strong>Given:</strong> Describe el estado/configuración inicial ',
                '<strong>When:</strong>  Especifica la acción o evento ',
                '<strong>Then:</strong>  Define el resultado esperado ',
                '',
                '- <strong>GWT</strong> funciona de la misma manera que <strong>AAA</strong>, pero con algunas ventajas ',
                '&bull; Puede ser usado con frameworks de BDD, es más que solo una estructura',
                '&bull; Es ubicuo por lo que más personas pueden entenderlo sin necesidad de un conocimiento completo del sistema ',
                '&bull; Añade valor extra al negocio, al interconectar el código y las pruebas a un nivel mayor ',
                '',
                '<strong>Test parametrizados:</strong> Permiten probar la misma lógica, con un conjunto determinado de entradas y salidas ',
                '- Además de que el test no se esta duplicando, también se agrupan diversos casos con condiciones similares ',
                '',
                '<strong>Mocks & Stubs:</strong> Generalmente las pruebas requieren que se ejecuten de manera aislada, sin interferencias ',
                '- Para lograr eso se <em>simulan</em> estas dependencias de manera que los tests cumplan los principios de <strong>FIRST</strong> y <strong>ISTQB</strong>',
                '- Son muy útiles pero se debe tener cuidado de no abusar de ellos, las pruebas pueden perder confiabilidad y acoplarse demasiado a la implementación ',
            ]),
        ...
        new Item(Keys.TESTING_TRIANGULO,
            [
                '- Los 3 elementos estan estrechamente relacionados, la modificación de alguno afecta a los otros 2 ',
                '',
                ...
                '                                                                                                                                     ',
                '   Costo         Tiempo       <strong>Costos:</strong> Recursos a gastar en el proyecto             + alcance   + tiempo    + costos ',
                '',
                '- Una gestión saludable de proyectos/productos dice que el cliente puede establecer 2/3 dimensiones, ',
                ' - <strong>PERO</strong> debe permitir que el equipo defina la tercera; <em>Ejemplos:</em>',
                '&bull; Si se establece el presupuesto y el alcance, el equipo puede determinar el tiempo necesario ',
                '&bull; Si se fija el tiempo y el alcance, debes flexibilizar el presupuesto ',
                '&bull; Si estableces el tiempo y el presupuesto, debes ceder con el alcance ',
                '',
                '- Cuando se presentan <em>"Triangulos imposibles"</em> la cuarta dimensión, que es la calidad, se ve afectada directamente ',
                '- Aun asi este tipo de proyectos son cotidianos y tambien logran finalizarse, con muchas deficiencias especialmente en la calidad ',
            ]),
        new Item(Keys.TESTING_COBERTURA,
            [
                '- La cobertura del código es fundamental para garantizar un alcance optimo de las pruebas ',
                ...
                '- Esto es importante ya que se puede usar para hacer despliegues e integración continua, y encontrar código muerto ',
                '- Existen varios tipos de cobertura, pero el más usado es la cobertura de las lineas de código ',
            ]),
        ...
        new Item(Keys.TESTING_DATA_CREATION,
            [
                ...
                '- Mejoran la legibilidad de los casos, al separar la creación de datos de la lógica de la prueba ',
                '- Permiten la flexibilidad y la reutilización de código, además de mejorar la mantenibilidad de las pruebas ',
                '',
        ...
        new Item(Keys.TESTING_FIVE_FACTORS,
            [
                ...
                '',
                '<strong>Verificar que el código funciona correctamente:</strong> Las pruebas dan confianza de que el código que se esta agregando/modificando ',
                ...
                ' - por problemas insospechados al haber modificado código sin hacer las respectivas pruebas ni haber tenido en cuenta posibles efectos segundarios ',
                '',
                ...
                '- El código es comunicación y aunque ya existen muchas practicas y formas para documentar el funcionamiento del sistema ',
                '- Las pruebas tambien son código y pueden ser diseñadas explicitamente con ese fin, y al ser ejecutables no se vuelven obsoletas con tanta facilidad ',
                '',
                '<strong>Proporcionar orientación sobre el diseño:</strong> Escribir pruebas genera tanta experiencia como escribir el código que se prueba ',
                ...
                '- El código especifico es util al principio, pero puede quedar facilmente obsoleto ó es dificil de mantener ',
                '- El código generalizado en cambio suele agregar complejidad a cambio de ser más util para el futuro ',
                '',
                '<strong>Apoyar la refactorización:</strong> La unica constante del software es el cambio, por lo que una base de código que sea evolucionable debe ',
                ' - tener un conjunto de pruebas que admita la refactorización mientras estas siguen comprobando la funcionalidad del código base ',
            ]),
        new Item(Keys.TESTING_BUG,
            [
                ...
                '&brvbar;--------------------------------------------------------------------------------------------------------&brvbar;',
                '&brvbar; Critica  &brvbar; Alto      &brvbar; Siempre         &brvbar; Errores de código          &brvbar; Funcional           &brvbar; Siempre    &brvbar;',
        ...
        new Item(Keys.TESTING_INTEGRATION,
            [
                '- Las aplicaciones cada vez son más complejas e interconectadas, la unión entre multiples aplicaciones se ha convertido en un indicador clave de exito ',
                '- Un software robusto y que satisfaga las necesidades del consumidor no es lograble sin que haya una fuerte coordinación entre las partes involucradas ',
                ...
                '',
                '- Tras corregir los problemas, se garantiza que tanto los componentes individuales como el sistema en conjunto operan correctamente ',
                '- También existen los <em>Slice Tests</em> que ejercen una sección transversal del sistema, probando capas o partes específicas de la aplicación ',
                '- Las pruebas de integración suelen ser el punto previo para realizar pruebas E2E y pruebas de sistema ',
                '',
                '- Pueden hacerse de forma incremental, comenzando desde módulos pequeños y avanzando progresivamente hasta cubrir el sistema completo ',
                '- El enfoque <em>Bottom-Up</em> es ideal cuando se agregan funcionalidades a un sistema existente, validando que lo nuevo y lo previo coexistan ',
                ...
                '',
                '- Enfoque <em>Big-Bang</em>: toda la aplicación se evalúa como una sola unidad, integrando y probando todos sus componentes de forma simultánea ',
                '- Requiere una comunicación estrecha entre los equipos de desarrollo y de pruebas para identificar y resolver problemas ',
                ...
                '',
                '- Las pruebas de integración suelen automatizarse debido a que son costosas e implican multiples componentes que no siempre son faciles de coordinar ',
                '- Los pipelines de <em>CI/CD</em> estan en capacidad de ejecutar estas pruebas de manera automatica como un paso previo obligatorio a cualquier despliegue ',
                ...
                '&bull; Validar el flujo de datos entre todas las partes del sistema, asegurando su integridad y que se transmitan de manera eficaz ',
                '&bull; Identificación de dependencias externas como APIs o sistemas de terceros, evitando problemas de compatibilidad ',
                '&bull; Definición de escenarios críticos personalizables, de manera que sean precisos y funcionen como puntos de control ',
                '&bull; Escalabilidad de manera que se puedan soportar la carga actual como el crecimiento a futuro ',
                '&bull; Detección temprana de inconsistencias, reduciendo riesgos antes de un despliegue '
            ]),
        new Item(Keys.TESTING_E2E,
            [
                '- No basta con tener una base fuerte de test unitarios, test de integración y tests sobre la interfaz gráfica ',
                '- Es necesario saber que todo el sistema funciona de <strong>Extremo a Extremo</strong>, y que cada punto de integración funciona correctamente ',
                '- Este tipo de pruebas no es infalible, y suele fallar por razones inesperadas o imprevisibles y son de las que más tiempo consumen en su elaboración ',
                '- En un entorno de microservicios surgen varias preguntas, como quién es el encargado de la prueba y qué tanto pueden abarcar ',
                '- También suelen requerir un gran mantenimiento y pueden demorarse bastante en su ejecución, por lo que suelen ser exclusivas para los flujos más críticos ',
                '',
                '- Entre más alto se suba en la pirámide de los tests, es probable entrar en el ámbito de probar características completas ',
                '- Esto es directamente útil ya que cambia la perspectiva del sistema y se usa como una caja negra, considerándose una prueba de <strong>aceptación</strong> ',
                '- Lo más importante de una prueba de aceptación es que satisfaga al cliente y no que solo funcione técnicamente ',
                '- Las pruebas de aceptación suelen estar ligadas a criterios definidos por el negocio y se alinean con <strong>BDD</strong>',
                '',
                '<strong>Consejos para escribir buenos tests</strong>',
                '&bull; La duplicación se debe seguir evitando, pero en este caso aplica sobre los diferentes tipos de pruebas, al igual que el codigo de producción, ',
                ' - el código de pruebas debe ser limpio, ya que este tambien requiere de tiempo para idearse, escribirse y ejecutarse ',
                '&bull; Si una prueba de alto nivel detecta un error, y no hay una prueba de bajo nivel que falle, ',
                ' - es necesario escribir una prueba de bajo nivel que cubra ese caso',
                '&bull; Las pruebas deben llegar a los niveles más bajos posibles en la piramide de pruebas, ',
                ' - si una prueba de bajo nivel cubre una de alto nivel, esta no aporta valor y puede ser eliminada ',
                '&bull; Se debe encontrar un equilibrio entre <em>Don´t repet yourself</em>(DRY) y <em>Descriptive and Meaningful Phrases</em>(DAMP)',
                '&bull; Puede aplicar la regla de 3 antes de considerar refactorizar, de manera que la abstracción resultante sea realmente justificada ',
            ]),
        ...
        new Item(Keys.TESTING_PROPERTY_TESTING_FUZZING_TESTING,
            [
                ...
                '- Cuando un caso activa áreas nuevas del código, estos casos se guardan para generar tests de mutación y así sucesivamente hasta que ',
    ...
    [
        ...
        new Item(Keys.ANALISIS_DISENO,
            [
                ...
                '<strong>Modularidad:</strong> Al ser modular esto facilitara la reutilización y que las futuras modificaciones afectan el menos código posible ',
        ...
        new Item(Keys.ANALISIS_MODEL_DOMAIN,
            [
                ...
                '- Se debe colocar en el foco la lógica de negocio, que sera el nucleo(core)',
        ...
        new Item(Keys.ANALISIS_TIME_COMPLEXITY,
            [
                '- Un algoritmo es una secuencia lógica de pasos, para resolver un problema especifico ',
        ...
        new Item(Keys.ANALISIS_HIGH_LEVEL_VS_LOW_LEVEL,
            [
                ...
                ' - Implica lógica real y detalles importantes para ser usados en la implementación ',
        ...
        new Item(Keys.ANALISIS_LIGHT_EVENTS_VS_RICH_EVENTS,
            [
                ...
                '&brvbar; Permiten manejar lógica empresarial         &brvbar; Mayores costos de infraestructura      &brvbar;                                  ',
                ...
                '- Un Evento de Dominio son importantes dentro del dominio empresarial e impulsan lógica de negocio ',
        ...
        new Item(Keys.ANALISIS_COHESERS_DECOUPLERS,
            [
                ...
                ' - que el código se mantenga unido(cohesión) mientras que otras buscan separarlo(desacoplamiento)',
                ...
                '- Generalmente se puede partir desde un código sin estructura, pasando por clases, modulos, servicios, monolitos modulares hasta microservicios ',
                ...
                '<strong>- Velocidad de desarrollo:</strong> Un programador que conoce su código es altamente productivo, pero esto tiene un limite, si deseas aumentar ',
                ...
                '<strong>- Seguridad:</strong> Un solo sistema siempre sera más fácil de proteger, sin embargo al crecer sus necesidades estas aumentan el perimetro ',
                '',
                '<strong>Fuerzas que empujan a fusionar el código:</strong>',
                ...
                '',
                '<strong>Fueras que Intentan mantener el código separado</strong>',
                '<strong>- Variabilidad:</strong> A mayor cantidad de requisitos que cumplir, una sola base de código unica no es apta ',
        ...
        new Item(Keys.ANALISIS_MULTI_BOUNDED_CONTEXT,
            [
                ...
                '- Gracias a su asociación directa con el código han sido asimilados y aplicados con relativa facilidad, mientras que otros más abstractos no tanto ',
        ...
        new Item(Keys.ANALISIS_STRATEGIC_DESIGN,
            [
                ...
                '',
                '- El <strong>Diseño Tactico</strong> se utiliza para realizar una transición gradual hacia el espacio de la solución a traves del código y sus transformaciones ',
                '',
                ...
                '&bull; Objetivo comercial claro, dividiendo el modelo heredado en partes lógicas que tengan un objetivo bien definido ',
        ...
        new Item(Keys.ANALISIS_TACTIC_DESIGN,
            [
                ...
                '- Al usar <strong>DDD</strong> suele enfocarse en la estructura rigida del código, objetos de dominio, estructuras y acciones sin contexto, ',
                ...
                '&brvbar; &bull; Exploración del dominio, descubriendo aspectos dinamicos de un dominio &brvbar; &bull; Falta de detalle estructurales de las estructuras internas del código     &brvbar; ',
    ...
    [
        ...
        new Item(Keys.SYSTEM_THINKING_I,
            [
                ...
                '',
                '<strong>Principios</strong>',
                '- El desarrollo de software es un trabajo en equipo; la arquitectura, el diseño y el código deben comunicarse de forma clara e intuitiva ',
        ...
        new Item(Keys.SYSTEM_THINKING_II,
            [
                ...
                '- Un sistema es más que código, o la suma de sus partes, es una ecosistema, en el cual la complejidad se va filtrando como agua en el cemento agrietado ',
        ...
        new Item(Keys.SYSTEM_DESIGN_SYSTEM_TYPES,
            [
                ...
                '&bull; Write Ahead Logs(WAL)                &bull; BD optimizadas para escritura             &bull; No planificar la lógica de reintentos o de la idempotencia ',
                ...
                '&bull; La proximidad espacial/semantica forman parte de la lógica de clasificación      &bull; Priorizar una latencia baja sobre alta precisión en los resultados ',
        ...
        new Item(Keys.SYSTEM_DESIGN_SCALING_WRITES,
            [
                ...
                '- Escalar la lectura mediantes caches, replicas de datos y DNS generalmente suele ser la parte más fácil ',
        ...
        new Item(Keys.SYSTEM_DESIGN_DATABASE_WITHOUT_LOGIC,
            [
                '- Las aplicaciones se dividen en múltiples capas que separan múltiples responsabilidades, depende de cada autor cuantas capas pueden existir ',
                '- Un tema común es la separación de la lógica, entre reglas de dominio (negocio) y la lógica de los datos (fuente de datos) ',
                '- Muchos desarrolladores suelen ver la DB como un almacenamiento de datos, que es mejor ocultar como un detalle de implementación ',
                '- SQL y NoSQL suelen aplicarse detras de frameworks, pero ocultar estas herramientas de gran utilidad evita aprovechar lo poderosas que son ',
                '',
                '&bull; <strong>Lógica Operativa:</strong> Este es el conjunto de reglas que se aplican constantemente y suele cambiar/evolucionar con frecuencia ',
                '&bull; <strong>Lógica de Datos:</strong> Lógica fundamental que no cambia con frecuencia, como tipos de datos o restricciones sobre los mismos ',
                '',
                '<strong>Consultas complejas</strong>',
                '- Todas las DB admiten SQL y todos sus comandos estandar, una de estas ventajas es poder realizar consultas complejas, a veces con lógica de negocio ',
                ' - algo que contradice los principios básicos de una arquitectura empresarial de capas ',
                '- Evitar estas consultas suele resultar en multiples consultas a la DB, consultas simples, que en la practica es un uso ineficiente de SQL ',
                '- Esta <em>"ineficiencia"</em> suele ser intencional para que la lógica sea fácil de leer y mantener ',
                '',
                '<strong>Modelo de dominio</strong>',
                '- Considerando un modelo de dominio clásico con POO, creando objetos que reflejan las tablas de la DB y sabiendo que en la practica ',
                ' - no siempre son replicas exactas, y que una vez cargados en memoria, estan disponibles para procesarse de acuerdo a la necesidad ',
                '- Esta separación permite la reutilización de las consultas para la carga de datos de otros procesos sobre este conjunto de objetos de dominio ',
                '',
                '<strong>Lógica en SQL</strong>',
                '- En los casos anteriores la DB solo almacena datos y aplica filtrados simples o medianamente complejos sobre los datos solicitados ',
                '- Usando SQL en toda su extensión seria capaz de realizar muchas operaciones, o todas las operaciones, alojadas en la capa de la aplicación ',
                '- Los cambios incompletos o sin procesar son más faciles de mantener en memoria, una vez persistidos es más dificil corregirlos o eliminarlos ',
                '',
                '<strong>Rendimiento</strong>',
                '- Es una pregunta importante, pero no es el principal factor a tener en cuenta, a menos que el volumen de datos lo requiera como un trade-off explicito ',
                '- El principal objetivo es que el código sea mantenible y despues de eso se podrian buscar problemas de redimiento mediante profiling o otras tecnicas ',
                '- Cuando la responsabilidad se encuentra en los metodos en memoria, estos siempre seran más ineficientes, sin importar la capacidad del equipo/servidor ',
                '- Una forma de optimizarlos es reescribir las consultas SQL de manera que se agrupen funciones y se reduzca la cantidad de viajes entre la aplicación y la DB ',
                '',
                '<strong>Mantenibilidad</strong>',
                '- El impacto a nivel de rendimiento mejora, a nivel del script es total, suele implicar reescrituras totales y el impacto crece si existen scripts similares ',
                '- En el modelo de dominio el impacto es más controlado, la lógica se adapta a las estructuras o la implementación, lo que debe hacer sigue siendo invariante ',
                '- Este es el punto principal entre escoger scripts transaccionales y la lógica de dominio, se suele compensar el acceso a la DB entre más lógica exista ',
                '',
                '<strong>Modificabilidad</strong>',
                '- Las aplicaciones suelen tener este punto muy presente, una aplicación empresarial compleja puede cambiar/evolucionar mucho a lo largo del tiempo ',
                '- El sistema debe estar organizado para permitir que esto se pueda lograr de manera fácil y rapida ',
                '- Esta suele ser tambien una razón principal para escoger la lógica de negocio en memoria, y solo aprovechar las fortalezas de la DB en puntos criticos ',
                '- SQL tambien presenta limitaciones, para lograr algoritmos realmente eficientes se suele requerir una codificación inteligente o fuera del estandar ',
                '',
                '<strong>Comprensibilidad</strong>',
                '- SQL suele considerarse complejo, bajo esta premisa muchos frameworks suelen afirmar, facilidad a los desarrolladores para no lidiar con ella ',
                '- Si la mayoria del equipo no se siente comodo con SQL es motivo de mantener la lógica de dominio alejada de SQL, y al mismo tiempo de capacitar el talento ',
                '- Aunque SQL es Turing-complete, muchos scripts suelen ser más complejos que sus contrapartes en lenguajes de programación imperativos y no declarativos ',
                '',
                '<strong>Duplicación</strong>',
                '- <strong>DRY</strong> No repetir código es uno de los principios de diseño más poderosos de la industria del desarrollo ',
                '- Si un dato especifico se necesita para funcionalidades de negocio independientes, los scripts empezaran a duplicarlo, ',
                ' - algo que no sucede tan fácil en los objetos de dominio ',
                '- En SQL esto se puede evitar mediante las Vistas, logrando el mismo objetivo de eliminar la duplicación ',
                '',
                '<strong>Encapsulación</strong>',
                '- Este es un principio base del Diseño Orientado a Objetos, y perfectamente aplicable al diseño del software en general ',
                '- En esencia establece que un programa debe dividirse en modulos que ocultan las estructuras de datos, tras una interfaz, ',
                ' - permitiendo cambios en el <em>como</em> sin afectar directamente al <em>que</em>, el equivalente en SQL podrian ser los procedimientos almacenados ',
                '- Una aplicación empresarial tiene esto presente y se esfuerza por separar la lógica de los datos, de la lógica del negocio, reduciendo el acoplamiento ',
                '- En este caso la pregunta es como encapsular la DB de manera que las modificaciones en los esquemas, no impliquen una cirugia a escopetazos ',
                '',
                '<strong>Portabilidad</strong>',
                '- Otra razón de peso para evitar SQL complejo es la portabilidad entre las DB, El SQL estandar es admitido por todos los proveedores ',
                '- Esta compatibilidad es teorica, ya que en la practica nunca se da al 100% y además hay que tener en cuenta que las migraciones ya no son tan complejas ',
                ...
                '- Esta decisión no puede ser evaluada desde un solo punto de vista, además de los mencionados, tambien se deben tener en cuenta los sesgos de cada programador ',
                '- Si las fuentes de datos estan separadas, el camino más sencillo sera mantener la lógica de dominio en memoria, como un punto unificador ',
                '- Si la fuente de datos es una sola, la comodidad del equipo toma relevancia, además de la mantenibilidad y la modificabilidad ',
                '- Si el rendimiento tiene problemas críticos, este se convierte en prioridad, no dude en cambiar acciones que están en memoria por consultas SQL ',
            ])
    ];
...
export const METODOLOGIAS =
    [
        ...
        new Item(Keys.METODOLOGIAS_SCRUM_PRINCIPLES,
            [
                '- Scrum es uno de los marcos más usados a la hora de desarrollar software, funciona como un marco para la gestion de proyectos, ',
        ...
        new Item(Keys.METODOLOGIAS_MODEL_BING_BANG,
            [
                ...
                '- Es fácil de gestionar ya que todas las etapas se hacen antes de entregar cualquier producto ',
            ]),
        ...
        new Item(Keys.METODOLOGIAS_MODEL_XP,
            [
                ...
                '- <strong>Revela la intención:</strong> El código debe ser <em>facil</em> de entender, debido a que el código es más leido que escrito ',
        ...
        new Item(Keys.METODOLOGIAS_MODEL_V,
            [
                ...
                '- Es un modelo fácil de seguir y el emparejamiento entre las etapas logra una mejor localizazion de fallos ',
...
export const TESTING_ADVANCED =
    [
        new Item(Keys.TESTING_ADVANCED_CAJA_NEGRA_CAJA_BLANCA,
            [
                '<strong>Caja Blanca:</strong> Son tests que se realizan sabiendo la estructura y funcionamiento interno del código ',
        ...
        new Item(Keys.TESTING_GRAFOS_CAUSA_EFECTO,
            [
                '- Estos grafos representan una relación lógica entre distinas causas y sus posibles efectos ',
        ...
        new Item(Keys.TESTING_ADVANCED_CASOS_DE_PRUEBA,
            [
                '- Entre más complejo sea un software, mayor sera la posibilidad de introducir defectos o de presentar errores ',
                '- Un caso de prueba es un conjunto de condiciones y variables bajo un entorno definido, que determinará si, ',
                ' - alguna funcionalidad cumple total o parcialmente con los criterios de aceptación ',
                '',
                '&bull; Los ciclos de pruebas deben ser continuos y eficientes ',
                '&bull; Deben estar escritos de manera clara y comprensible ',
                '&bull; Tener las entradas y salidas debidamente especificadas ',
                '&bull; La ejecución debe ser fácil/reproducible o estar guiada de no poder serlo ',
                '&bull; Definir los comportamientos esperados y como invocarlos',
                '&bull; Debe existir al menos un caso de prueba por cada requisito definido',
                '',
                '<strong>Estados</strong>',
                '<strong>- Exitoso:</strong> Todos los pasos y el comportamiento han sido los esperados ',
                '<strong>- Fallido:</strong> Si alguno de los paso falla durante la ejecución o el resultado no es el esperado ',
                '<strong>- Bloqueado:</strong> Si este no se puede realizar debido a fallos previos o dependencias faltantes ',
                ...
                '&bull; Identificar los requisitos a probar y enumerar los casos de cada requisito ',
                '&bull; Realizar una matriz de trazabilidad para vincular los requisitos con los casos de prueba y sus posibles defectos ',
                '&bull; Redactar el objetivo del caso, que permita a personas con o sin conocimientos previos entender la prueba ',
                ...
                '&bull; Documentar los resultados, sean satisfactorios o no ',
            ]),
        new Item(Keys.TESTING_ADVANCED_CORNER_AND_EDGE_CASE,
            [
                '- Son 2 conceptos diferentes, aunque comúnmente se usan o nombran de manera equivalente ',
                '',
                '<strong>Edge case:</strong> Un caso límite sucede cuando un problema o comportamiento inesperado se produce en un parámetro de funcionamiento extremo (maxímo/mínimo) ',
                '- Los valores limites no siempre implican que haya situaciones problematicas, pueden ser casos conocidos y anticipables ',
                '',
                '<strong>Corner case:</strong> Un caso excepcional sucede cuando múltiples parámetros estan con valores límites, formando una intersección de los limites ',
                '- La intersección de límites puede exponer comportamientos que los <em>edge cases</em> individuales no revelan ',
                '',
                '<em>Ejemplo:</em> Supongamos que se tiene un horno con 2 configuraciones(parámetros) ',
                '&bull; Temporizador [0.5, 10] Minutos ',
                '&bull; Temperatura  [10, 300] Grados Centigrados ',
                '',
                '<strong>Caso Límite:</strong> El tiempo se encuentra en los valores máximos(10 min) o mínimos(0.5 min)',
                '<strong>Caso Excepcional:</strong> Temperatura en su máximo(300°C) y temporizador en su máximo(10 min) de forma simultánea ',
            ]),
        ...
        new Item(Keys.TESTING_ADVANCED_SMELLS_CODES1,
            [
                ...
                '<strong>- Well, My Setup Works:</strong> El test se puede desviar o duplicar el código de produccion innecesariamente por no compartir configuraciones ',
                ...
                '<strong>- Sobre refactorizar:</strong> Se ha aplicado tando DRY al código de pruebas que ha perdido significado y legibilidad ',
                '<strong>- Boilerplate hell:</strong> No se puede entender el tests ya que hay demasiado código, indicando la falta de fabricacion de datos ',
                ...
                '<strong>- Automatizacion:</strong> Si el código de tests es autogenerado, es necesario revisarlo para saber que no es inutil o esta mal ',
                '',
                '<strong>Datos</strong>',
        ...
        new Item(Keys.TESTING_ADVANCED_SMELLS_CODES2,
            [
                ...
                '<strong>- Bumbling assertions:</strong> Se escogen a voluntad afirmaciones poco sofisticadas y de fácil verificacion ',
                '<strong>- Assertion diversion:</strong> Cuando se escoge la asercion erronea para que el fallo de la prueba sea más fácil de entender ',
                ...
                '<strong>- Conditional assertions:</strong> Sobreesfuerzo en la verificacion donde las aserciones son dinamicas de acuerdo a la ejecucion del código de pruebas ',
                ...
                '<strong>- The True Believer:</strong> La prueba convence al autor de que el código esta bien y no que tal vez esta erroneo ',
                ...
                '<strong>- The telltale heart:</strong> Cuando existe código de produccion que solo cobra sentido en la ejecucion de los tests ',
                '',
                ...
                '<strong>- Is There Anybody There ? </strong> Prueba o código inestable que dañan la compilacion ocasionalmente ',
                ...
                '<strong>- Contortionist testing:</strong> Debido a que las pruebas fueron escritas despues del código, este debe esforzarse para funcionar ',
                '<strong>- The Hans Moretti Sword Box:</strong> Entre mas tests se agreguen a los puntos testeables, menos se evitaran las vias no descubiertas del código ',
            ]),
        new Item(Keys.TESTING_ADVANCED_MUTATION,
            [
                ...
                '- Una mutación es una prueba con una ligera modificación del código, si estas llegan a pasar el mutante sobrevivio, por lo que el código esta mal ',
                '- Entre menos mutaciones sobrevivan, mejor sera la calidad del código probado ',
                '- Tambien se ha de tener en cuenta que pueden existir mutantes equivalentes, que hacen lo mismo que el código original, solo que con otra sintaxis ',
                '',
                '<strong>Casos:</strong>',
                '- Cambiar valores usados en el código original, como empezar un ciclo en 1 y no en 0, etc... ',
                '- Eliminar, intercambiar o duplicar bloques de código declarados ',
                '- Decisiones al comparar valores, ya sea intercambiando un == por un != por ejemplo',
                '',
                '<strong>Ventajas:</strong>',
                '- Permiten cubrir gran parte de la lógica a probar ',
                '- Detecta errores de fácil alusión ',
        ...
        new Item(Keys.TESTING_ADVANCED_CHARGE,
            [
                ...
                '- Creación de scripts de prueba ya sean mediante código ó interfaz grafica, de manera que se puedan simular escenarios reales ',
        ...
        new Item(Keys.TESTING_ADVANCED_ARCH_UNIT,
            [
                ...
                '- Siempre puede existir código que no se adhiera a nuestras reglas arquitectonicas ó que este fuera de contexto ',
                '- En este caso dentro del amplio contexto que involucra la arquitectura, hace referencia a la organización en paquetes de nuestro código ',
                '',
                ...
                '- Uso de anotaciones indebidas                - Mantener convenciones de código',
            ]),
        new Item(Keys.TESTING_ADVANCED_ENVIRONMENT,
            [
                ...
                '- Cuando se tienen pipelines y/o CI/CD estos agregan ó omiten pasos y etapas segun el código y en que ambiente se va a desplegar ',
                '',
                '<strong>Desarrollo:</strong> Ambiente donde se crean o actualizan las aplicaciones por parte de los desarrolladores ',
                '- Suele ser un ambiente compartido por lo que el código de multiples desarrolladores puede llegar a coexistir ',
                '',
        ...
        new Item(Keys.TESTING_ADVANCED_DEFECT_RATE,
            [
                '- Crear software de calidad va más alla de tener una base de codigo solida y bien estructurada, además de generar una buena experiencia con el cliente ',
                '- Controlar la cantidad y el impacto de los defectos tambien mejora otros aspectos, como el retorno de la inversión (ROI)',
                '- Entre más tarde se encuentre un defecto y entre más tarde se corrija, mayor sera el costo asociado y los posibles retrasos adicionales ',
                '',
                '<strong>¿ Que es un defecto ?</strong> Son problemas en un producto de software que impiden cumplir las expectativas del cliente o segun los requisitos ',
                '- Esto puede escalar desde defectos menores, como errores tipograficos, hasta errores graves que interrumpen la operación ',
                '- Gestionar los defectos es desafiante, debido a que el software suele ser complejo y los problemas pueden escalar rapidamente ',
                '',
                '<strong>¿ Que es un error ?</strong> Es un fallo humano durante la codificación que provoca que el software se comporte de forma inesperada o que produzca resultados incorrectos ',
                ' - los errores suelen estar relacionados a acciones o omisiones humanas, como logica incompleta o casos extremos (edge cases)',
                '- Esto se puede reducir desde la raiz, mediante revisiones estrictas de codigo y analisis estatico ',
                '',
                '<strong>¿ Que es un incidente ?</strong> Es una interrupción causada por un defecto o factores externos, que afecta el funcionamiento normal del sistema ',
                '- Para evitar estas repercusiones, el proceso de pruebas debe ser riguroso y la calidad debe ser un proceso continuo, más que una sola etapa ',
                '',
                '<strong>Tasa de defectos:</strong> Es el porcentaje de defectos detectados en el software, durante el desarrollo o despues de su lanzamiento ',
                ' - en relación con el trabajo total entregado, es un indicador clave para comprender la incidencia en el proyecto, y la calidad general ',
                '- Su seguimiento proporciona una vista clara de la calidad del producto, ayudando a identificar problemas y la toma de medidas correctivas ',
                '- Es importante mantener una tasa aceptable de defectos, un ideal seria cero, pero no siempre sera alcanzable ',
                '',
                '<strong>¿ Porque es importante ?</strong> <em>Defect Rate = (cantidad de defectos / entregables) * 100 </em>',
                '&bull; <strong>Satisfacción del usuario:</strong> Entre más errores lleguen a producción, más se reduce la confianza del usuario final ',
                '&bull; <strong>Eficiencia:</strong> Una tasa alta revela brechas en las pruebas realizadas, revisiones de codigo ó en el proceso de calidad ',
                '&bull; <strong>Impacto financiero:</strong> La mala calidad es costosa, reducir los defectos ayuda directamente a reducir casos de soporte y reprocesos ',
                '',
                '<strong>¿ Que es un entregable ?</strong> Se refiere al trabajo total que haya sido completado durante un periodo especifico, ',
                ' - pero esto esta sujeto a cada flujo de trabajo, pueden usarse lineas de codigo, puntos de historia, historias de usuario ',
                '- Sin importar que herramienta se use para la medición y de que escoja para la formula; el panorama general no debe perderse de vista ',
                '',
                '<strong>Metricas:</strong> No solo se debe usar la formula de la tasa de defectos, existen más metricas complementarias que ayudan a monitorear más areas y su calidad ',
                '&bull; <strong>Densidad de defectos:</strong> Indica si los defectos se concentran en partes especificas, de esa manera se puede saber donde enfocar las mejoras ',
                '&bull; <strong>Tiempo de solución:</strong> Entre más prolongados sean, pueden indicar ineficiencias en el proceso, falta de recursos o demasiada deuda tecnica ',
                '&bull; <strong>Change failure rate(DORA):</strong> Una tasa alta indica problemas en el proceso en general, calidad del codigo, pruebas, implementación, estabilidad ',
                '&bull; <strong>Recurrencia de defectos:</strong> Si los problemas persisten, puede indicar problemas de fondo o más graves, ',
                '&bull; <strong>Lead time for changes(DORA):</strong> Esta metrica mide cuanto tiempo tardan los cambios desde la implementación, hasta llegar al entorno productivo ',
                '',
                '<strong>Tipos de defectos:</strong> No todos son iguales y comprender cada tipo ayuda a tomar medidas adecuadas ',
                '&bull; <strong>Defectos funcionales:</strong> Son los más criticos debido a que afectan el funcionamiento normal de la operación ',
                '&bull; <strong>Defectos de rendimiento:</strong> Afectan la experiencia al hacer que responda de manera lenta, que se quede bloqueado o exista degradación bajo carga ',
                '&bull; <strong>Defectos UI/UX:</strong> Afectan la usabilidad o el diseño visual de la aplicación, como navegación confusa o diseños inconsistentes ',
                '&bull; <strong>Defectos de seguridad:</strong> Amenazas que exponen al sistema, como autenticación debil o almacenamiento de datos inseguro ',
                '&bull; <strong>Defectos de regresión:</strong> Ocurren cuando las actualizaciones anteriores dejan de funcionar, o nuevas funcionalidades provocan nuevos errores ',
                '',
                '<strong>Mejorar la tasa de defectos:</strong> Esto implica más que solucionar problemas y mejoras en el enfoque del desarrollo, ',
                ' - detección temprana de defectos, mejor colaboración, y lograr pruebas más eficientes y/o inteligentes ',
                '&bull; Mejorar las practicas de pruebas, como la automatización y el uso de herramientas que faciliten las pruebas manuales ',
                '&bull; Adoptar el enfoque Shift-Left y comenzar a hacer pruebas desde etapas tempranas del SDLC ',
                '&bull; Revisiones de codigo, que ayudan a detectar desde inconsistencias menores, hasta errores significativos ',
                '&bull; El monitoreo de metricas de manera regular, ayuda a anticiparse a las tendencias y darle seguimiento a los problemas actuales ',
                '&bull; Reducir el trabajo en progreso (WIP), esto ayuda al equipo a concentrarse mejor, sin cambiar de contexto y completarlas con estandares más altos ',
            ])
    ];
