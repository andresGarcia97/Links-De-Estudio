    ...
    [
        ...
        new Item('debugging',
            [
                ...
                ' - Preste atención a los mensajes de error -> si el codigo falla, casi todos los lenguajes son capaces de demostrar el porque de este fallo, ',
                '  - algunos con más exactitud que otros, enteder el error es la base para encontrar una solución ',
                ' - Buscar en internet -> si no conoce el error, o no sabe el porque de este, es probable que en internet ya exista una explicación, ',
                '  - sea explicando el error, o dando soluciones para este, en contextos similares ',
                '',
                ' - Usar fuentes confiables y actualizadas -> siempre es buena idea leer la documentación oficial para entender la raiz del problema, ',
                '  - tambien saber que no todas las soluciones no son optimas, ya que pueden estar desactualizadas ó no funcionarian en todos los ambientes ',
                ' - Explica tu logica -> existe la posibilidad de que se haya malinterpretado la información ó se esten haciendo suposiciones erroneas, ',
                '  - por lo que revisar tu codigo con un pato de goma, explicandolo linea por linea, podemos comprender cosas que antes no ',
                '',
                ' - Reduzca las posiblidades -> si un codigo funciona de cierta manera, deberiamos procurar que siga funcionando asi, por ejemplo ',
                ...
                ' - Tomar un descanso -> Cuando los bugs son más grandes y complejos es posible que su resolución conlleve mucho tiempo, por lo que ',
                ...
                ' - Buscar ayuda -> cuando los problemas son demasiado grandes o complicados, preguntarle a alguien con más experiencia es de los más normal, ',
                ...
                '',
                '- Como evitarlos: ',
                ' - Codigo limpio -> tenga en cuenta que los principios como SOLID, KISS, DRY y muchos más estan ahi para mejorar nuestro codigo, ',
    ...
    [
        ...
        new Item('interprete',
            [
                ...
                ' ',
                ' - Interpretados -> Estos deben tambien deben ser traducidos, a traves de un intermediario ',
    ...
    [
        ...
        new Item('leyesInternet',
            [
                ...
                '',
                '- Ley de Skitt -> "Cualquier publicación que corrija un error en otra publicación contendrá al menos un error" ',
                '- Ley de Godwin -> "A medida que una discusión online se alarga, la probabilidad de que aparezca una comparación a Hitler o los nazis tiende a 1" ',
                '- Ley de Poe -> "Sin un emoticón u otra muestra de humor, es absolutamente imposible parodiar una idea sin que alguien la confunda con una opinión genuina" ',
                '- Ley de Cunningham -> "La mejor manera de conseguir una respuesta correcta en la red, no es haciendo la pregunta sino publicando una respuesta equivocada" ',
                '- Ley de Exclamación -> "Cuantos más signos de exclamación se usen, más probable es que sea una mentira. También aplica para el uso excesivo de mayúsculas" ',
                ...
                '- Regla 34: -> "Si existe, hay pornografía" ',
                '- Regla 35: -> "Si no existe tal pornografía, se hará"',
                '- Ley de Pommer -> "La opinión de una persona puede cambiar en base a lo que lee en internet. La naturaleza del cambio será: de no tener, a tenerla equivocada" ',
        ...
        new Item('pareto',
            [
                '- Tambien conocida como la regla de 80/20 que asegura que el 80% de las causas proviene del 20% del esfuerzo ',
    ...
    [
        ...
        new Item('canvas',
            [
                ...
                '<strong> - Segmento de mercado:</strong> ¿ Quienes son tus clientes ?, son la base de todo tu negocio',
                '<strong> - Propuesta de valor:</strong> Tu diferencia frente a la competencia como ventaja competitiva',
                '<strong> - Canales:</strong> Como vas a distribuir tu producto',
                '<strong> - Comunicación:</strong> Como te comunicaras con tus clientes',
                '<strong> - Ingresos:</strong> Como ganaras dinero y de que fuentes',
                '<strong> - Recursos:</strong> Cosas como derechos de autor, personal, fondos, patentes, etc..',
                '<strong> - Actividades:</strong> Producción, soluciónes, plataformas y todo lo necesarios para tu propuesta ',
                '<strong> - Socios:</strong> Quienes te ayudaran a conseguir los recursos necesarios',
                '<strong> - Costos:</strong> Definir cuanto gastar y como gastarlo',
            ]),
...
export const NUBE =
    [
        ...
        new Item('tiposApi',
            [
                '- Locales -> ofrecen servicios del SO o como middleware entre apliaciones, como el acceso a una BD ',
                '- Web -> ampliamente utilizadas para acceder a recursos expuestos mediantes el protocolo HTTP mediante una URL',
                ' - estas generalmente son de tipo REST pero tambien existen con el protocolo SOAP ',
                '- Programación -> se basan en llamar procedimientos remotos y que aparenten ser locales, ',
        ...
        new Item('capTeorema',
            [
                ...
                '- Partitions (tolerancia a la particion) -> el sistema sigue funcionando como un todo, despues de que las partes fallen o se incomuniquen ',
                '',
    ...
    [
        ...
        new Item('desglosegrasp',
            [
                ...
                '',
                '- Invención pura -> son clases que no representan un objeto o una abstracción especifica del dominio, ',
                ...
                '',
                '- Experto en Información -> la responsabilidad de crear o usar un objeto solo es de aquel que conoce todo lo necesario, ',
                ' - reduciendo el acoplamiento, aumentando la cohesión y encapsulamiento ',
                '',
                '- Controlador -> sirve como intermediario entre una determinada interfaz y el algoritmo que la implementa, ',
    ...
    [
        ...
        new Item('codeSmellsTests1',
            [
                ...
                ' - Identidad equivocada -> cuando el nombre de una prueba es contrario a lo que esta haciendo ',
                ' - Caso anonimo -> cuando su nombre no describe lo que se esta probando ',
                ' - Testy test -> cuando una prueba tiene la redundancia de que indica que es una prueba ',
                ' - Ni verbos ni sustantivos -> esta tiene el nombre del concepto pero no del comportamiento previsto ',
                '',
                '- Estructurales: ',
                ' - Todo es una propiedad -> cuando variables temporales se mantienen como variables de instancia ',
                ' - Falta de parametrizacion -> cuando se llaman metodos fuera del metodo de tests o se hace un 2x1 en un mismo test ',
                ' - Test de integracion enmascarado -> demasiadas capas estan involucradas en un test ',
                ' - El Parasito -> los tests deben ser independientes y nunca depender de otros tests previos ',
                ' - Pruebas cuajadas -> union inapropiada de varios tests o division innecesaria ',
                '',
                '- Inicializacion y desmontaje (setup, teardown)',
                ' - Primeros y ultimos ritos -> metodos repetivos en cada prueba al iniciar y finalizarla, en vez de una configuracion comun ',
                ' - sobre configuracion -> uso excesivo de datos compartidos que no todos los tests necesitan ',
                ' - La configuracion esta en otra parte -> y la prueba solo hace las validaciones ',
                ' - Mi configuracion funciona -> el test se puede desviar o duplicar el codigo de produccion inneceriamente ',
                ' - Data-ja vu -> Los datos que pueden ser inmutables y compartidos se instancian innecesariamente con cada test ',
                ' - Limpieza dolorosa -> cuando se necesita crear o destruir un recurso cosotos y la separacion del test con este es debil o muy larga ',
                '',
                '- Codificacion: ',
                ' - Ocultamiento -> cuando algo que es parte fundamental del test esta como comentario pero no codificado ',
                ' - sobre refactorizar -> se ha aplicado tando DRY al codigo de pruebas que ha perdido significado y legibilidad ',
                ' - Infierno repetivo -> no se puede entender el tests ya que hay demasiado codigo, indicando la falta de fabricacion de datos ',
                ' - Metodos incompletos -> cuando hay metodos de utilidad que no preparan todas las cosas como las necesita el test ',
                ' - Automatizacion -> si el codigo de tests es autogenerado, es necesario revisarlo para saber que no es inutil o esta mal ',
                '',
                '- Datos: ',
                ' - ¿ que se prueba ? -> los datos del tests no son claros en lo que son o hacen ',
                ' - Fabrica de datos perdidad -> cuando cada tests construye sus propios datos en vez de tener un metodo comun para esto ',
                ' - Datos mundanos -> son datos que sirven para el contexto de la prueba pero no son reales ',
                ' - Rueda de la fortuna -> los datos aleatorios pueden conducir al fallo de las pruebas, haciendolas erraticas ',
                ' - Bomba de tiempo -> cuando se prueban informacion con fechas, si no se tiene cuidado estos datos pueden volverse erraticos ',
            ]),
        new Item('codeSmellsTests2',
            [
                '- Aserciones - frameworks: ',
                ' - Chat logging -> se usan mensajes por consola o logs para suplir el fallo de una asercion ',
                ' - Afirmaciones torpes -> se escogen a voluntad afirmaciones poco sofisticadas y de facil verificacion ',
                ' - Desviacion -> cuando se escoge la asercion erronea para que el fallo de la prueba sea más facil de entender ',
                ' - Afirmar el mazo -> se crean aserciones demasiado completas para no pensar en una mas general e indicativa ',
                ' - Asercion Martini -> en vez de verificar todo, se verifica lo que sea y como sea para que el test pase ',
                ' - Condicionales -> sobreesfuerzo en la verificacion donde las aserciones son dinamicas de acuerdo a la ejecucion del codigo de pruebas ',
                ' - Confusas -> La falta de control en el sistema conlleva a no predecir resultados exactos si no coincidencias parciales ',
                '',
                '- pruebas: ',
                ' - EL verdadero creyente -> la prueba convence al autor de que el codigo esta bien y no que tal vez esta erroneo ',
                ' - Asertar el mundo -> las aserciones prueban todo, hasta detalles sin sentido ',
                ' - Evidencia circunstancial -> las aserciones comprueban comportamientos indirectos ',
                ' - intermitentes -> las afirmaciones fallan en comprobar el panorama general al centrarse en subconjuntos de detalles ',
                '',
                '- Locura por los mocks: ',
                ' - sobremockear -> son pruebas de caja blanca que siempre pasan ya que todo es perfecto ',
                ' - locura simulada -> cuando se abusa de los mocks hasta para crear simples objetos primitivos como POJOs',
                ' - Simulacion de control remoto -> la clase que depende de un servicio se prueba con la dependencia simulada y no el servicio en si ',
                ' - El corazon revelador -> cuando existe codigo de produccion que solo cobra sentido en la ejecucion de los tests ',
                '',
                '- Inestables: ',
                ' - El cienpies humano -> no solo sucede cuando existe una prueba dependediente de otra, si no que hay una secuencia especifica para que no fallen ',
                ' - ¿ Hay alguien ahi ? -> prueba o codigo inestable que dañan la compilacion ocasionalmente ',
                ' - Caldero chorreante -> prueba sensible a condiciones creadas por otras pruebas o que corrompe a otras ',
                ' - Solista -> solo funciona cuando nadie más se esta ejecutando ',
                ' - Demandante de recursos -> si el sistema esta lento este test fallara, debido a procesos internos pesados ',
                '',
                '- Implementacion: ',
                ...
                ' - Caja de espadas -> entre mas tests se agreguen con los diversos puntos testeables, menos se evitaran las vias no descubiertas del codigo ',
            ]),
        new Item('mutation',
            [
